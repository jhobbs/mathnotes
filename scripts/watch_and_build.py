...
def should_ignore(path: str) -> bool:
    ...
    # generated by the build itself (committed like a lockfile); reacting to
    # it would restart the watcher after every notation change
    if name == 'mathnotes-notation.sty':
        return True
...
def requires_restart(changed: list) -> bool:
    ...
    imported), so the watcher must re-exec itself to pick them up. Same for
    mathnotes.sty: latex_processor (PRE-EXPANSION MACROS) and the MathML
    worker (MATH MACROS) each read it once per process."""
    return any(path.endswith(('.py', '.sty')) for path in changed)
