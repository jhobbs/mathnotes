...
A point $z_0$ is called a \textbf{singularity} of a \@{complex-function} $f$ if $f$ is not \@[analytic]{analytic-at-a-point} at $z_0$, but every \@{neighborhood} of $z_0$ contains at least one point at which $f$ is analytic.
\end{definition}
...
\begin{example}
Consider $f(z) = z$ defined in the \@{punctured-plane}. Then, the \@{origin} is an \@{isolated-singularity}, because $f(z)$ is not defined there, but is defined everywhere else in a \@{neighborhood} of the origin. We can easily define $f(0) = 0,$ and this will make $f(z)$ \@[analytic]{analytic-at-a-point} at $0,$ and so we say this \@{singularity} is \@{removable}.
\end{example}
