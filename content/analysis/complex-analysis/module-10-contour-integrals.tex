...
A contour integral is essentially a \pagelink[line integral]{analysis/calculus/line-integrals} in the complex plane.

\begin{definition}[Contour Integral]
Given a smooth curve $C$ in a domain $D$ and $f(z) = u(x,y) + v(x,y)i$ a \dref{continuous} \dref{function} defined along $C$, we say that
...
\includegraphics[alt={Curve Path}]{contour.png}
...
\subsection{Some Important Contour Integrals}
...
\begin{theorem}\label{integral-of-one-over-z-around-unit-circle}
...
Now, using the definition of the \dref{contour-integral},
