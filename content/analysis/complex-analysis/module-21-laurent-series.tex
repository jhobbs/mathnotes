...
\textbf{Laurent Expansion Theorem}. Let $f$ be analytic in an annulus $D : r < \|z - z_0\| < R$. Then $f(z)$ can be expressed in the form
...
\includegraphics[alt={Annular disc.png}]{annulus.png}
...
\section{General Method for Finding all Taylor and Laurent Series}

If we want to find all of the Taylor and Laurent Series for $f(z)$ centered at $z_0$ we can do this:

\subsection{1. Consider where there are singularities.}

If the is a disc around the center that contains no singularities, then we'll have a \dref{taylor-series} centered at that disc, and its region of convergence will extend until the first singularity is encountered.
...
\subsection{4. Translate back to $z$ land}
