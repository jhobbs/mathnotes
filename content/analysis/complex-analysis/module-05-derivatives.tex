...
\title{Derivatives of Complex Functions}
...
The \textbf{derivative} of a \dref{complex-function} $f$ with respect to $z$ at $z_0$ is defined as

\[ f'(z_0) = \lim_{\Delta z -> 0} \frac{f(z_0 + \Delta z) - f(z_0)}{\Delta z}, \tag{a} \]

provided the \dref{limit} exists.
\end{definition}

\begin{definition}[Complex Differentiable]
In the above definition of \dref{complex-derivative}, if the \dref{limit} exists, $f$ is said to be \textbf{differentiable} at the \dref{point} $z_0$.
\end{definition}

\begin{remark}
When a \dref{function} is \dref{complex-differentiable} at a point $z_0$, then it is \dref{continuous} at $z_0$, however, the converse is not necessarily true - $\Re{(z)}$ is \dref{continuous} on all of $C$ but is nowhere \dref{complex-differentiable}.
\end{remark}

Because the \dref{complex-derivative} is defined using a \dref{limit}, and we need the \dref{limit} to be the same when approaching a \dref{point} from any direction, we generally only consider \dref{interior-points}. That is, if a \dref{function} $f(z)$ is defined at \dref{points} \dref{interior} to and on a curve $C$, then $f'(z)$ is only considered at points interior to $C$. This is automatically the case when the \dref{domain-of-definition} of $f(z)$ is an \dref{open} set.

\begin{definition}[Analytic]
We say that a \dref{complex-function} $f$ is \textbf{analytic} in an open set $S$ if it has a \dref{complex-derivative} at every \dref{point} of $S$. In other words, when studying \dref{complex-functions}, we consider differentiability on \dref{open} \dref{sets} rather than on specific \dref{points}.
\end{definition}

We can still talk about points!

\begin{definition}[Analytic at a point]
A \dref{complex-function} $f$ is said to be \textbf{analytic at a point} $z$ if it is \dref{analytic} in some \dref{neighborhood} of $z$. However, even here, the \dref{point} being \dref{analytic} depends on the \dref{neighborhood} around the point being \dref{complex-differentiable}.
\end{definition}

\begin{definition}[Entire]\synonyms{entire-function}
A function which is analytic on the whole complex plane is called an \textbf{entire function}.
\end{definition}
...
which defines the \dref{complex-derivative} of $f$ at the \dref{point} $z$. This equation defines the \dref{complex-function} $f'$, known as the \textbf{derivative function.}
\end{definition}
...
Moreover, it's not yet clear how to tell when a \dref{complex-combination} $u(x,y) + v(x,y)i$ of \dref{real} \dref{functions} $u(x,y)$ and $v(x,y)$ define a \dref{function} $f(z)$ of $z = x + yi$.
...
\begin{theorem}[Cauchy-Riemann Equations]\label{cauchy-riemann-equations}
A function $f(z) = u(x,y) + v(x,y)i$ is \dref{analytic} in an \dref{open} \dref{set} $S$ if and only if the first \dref{partial-derivatives} of $u(x,y)$ and $v(x,y)$ are \dref{continuous} on $S$ and satisfy the Cauchy-Riemann equations therein
...
\begin{theorem}\label{analytic-implies-cr-equations}
Assume a \dref{complex-function} $f(z)$ is defined on a \dref{neighborhood} of $z_0,$ and $f$ is \dref{complex-differentiable} at $z_0.$ Let $z = x+iy$ and write $f(x +iy) = u(x,y) + iv(x,y).$ Then all of the \dref{partial-derivatives} $u_x, u_y, v_x, v_y$ exist and
...
\begin{proof}
By assumption and definition of the \dref{complex-derivative}, we have that
...
This gives us that the \dref{partial-derivatives} $u_x$ and $v_x$ exist and
...
This gives us that the \dref{partial-derivatives} $u_y$ and $v_y$ exist and
...
See \href{https://personal.math.ubc.ca/~feldman/m300/cauchyRiemann.pdf}{this page for the proof of the converse theorem}.
...
That is a \dref{vector} in the $uv$-plane (the output plane) pointing in the direction $\arg(f'(z)).$
...
That's is a \dref{vector} in the $uv$-plane as well, with the same magnitude as the vector in (c), but in the direction $i \arg(f'(z)),$ that is, (d) has the same magnitude as (c), but is rotated by 90 degrees as its multiplied by $i.$

So, the two vectors are of equal magnitude and are orthogonal.
\end{intuition}

\includedemo{cauchy-riemann}
