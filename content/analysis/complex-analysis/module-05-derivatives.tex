...
The \textbf{derivative} of a \@{complex-function} $f$ with respect to $z$ at $z_0$ is defined as
...
In the above definition of \@{complex-derivative}, if the \@{limit} exists, $f$ is said to be \textbf{differentiable} at the \@{point} $z_0$.
\end{definition}
...
We say that a \@{complex-function} $f$ is \textbf{analytic} in an open set $S$ if it has a \@{complex-derivative} at every \@{point} of $S$. In other words, when studying \@{complex-functions}, we consider differentiability on \@{open} \@{sets} rather than on specific \@{points}.
\end{definition}

We can still talk about points!

\begin{definition}[Analytic at a point]
A \@{complex-function} $f$ is said to be \textbf{analytic at a point} $z$ if it is \@{analytic} in some \@{neighborhood} of $z$. However, even here, the \@{point} being \@{analytic} depends on the \@{neighborhood} around the point being \@{complex-differentiable}.
\end{definition}
...
which defines the \@{complex-derivative} of $f$ at the \@{point} $z$. This equation defines the \@{complex-function} $f'$, known as the \textbf{derivative function.}
\end{definition}
