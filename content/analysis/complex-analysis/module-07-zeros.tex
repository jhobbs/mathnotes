...
:::define "Zero"
A point $z_0$ is called a \textbf{zero} of an order $m$ of a complex function $f$ if $f$ is analytic at $z_0$ and

\[ 0 = f(z_0) = f'(z_0) = \cdots = f^{(m-1)}(z_0), f^{(m)}(z_0) \neq 0. \]
:::

:::define "Simple Zero"
Zeroes of order 1 are often called \textbf{simple zeros}.
:::
...
\begin{intuition}
We can see this from the definition of the \dref{taylor-series}. Assume we're at a zero, that is, that $f(z_0) = 0.$ Then
