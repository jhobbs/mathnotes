...
\subsection{Full Period Trigonometric Integrals}
...
We then can proceed with a normal contour integral, using \dref{residue} methods to deal with any singularities enclosed by the contour.
...
where $f(x)$ is a \dref{rational-function}. It can also be used with integrals of the form

\[ \int_0^{\infty} f(x) dx \]

where $f(x)$ is both \dref{rational} and \dref{even}.
...
\includegraphics[alt={Improper Contour Integral}]{improper.png}
...
then the semi-circle grows to enclose the entire upper half-plane. If the value of the complex contour integral along $S$ goes to $0$ as $R$ goes to infinity, then the contour integral is equivalent to the integral along the real axis, and we can use \dref{residue} methods to compute it, accounting for any singularities in the upper half-plane.
...
Using the \dref{residue-theorem}, we have then that
...
\subsection{Improper Integrals with Simple Poles on the Real Axis}
...
This \dref{limit} is called the \textbf{Cauchy principal value} of the integral and is written as

\[ \text{pr. v. } \int_A^{B} f(x) dx. \]
\end{definition}

This principal value can exist, although the integral itself is undefined. We can use this for the following theorem in the case there are \dref{simple} \dref{poles} on the real axis and we have an integral from $-\infty$ to $\infty.$

\begin{theorem}[Simple Poles on the Real Axis]
If $f(z)$ has a \dref{simple} \dref{pole} at $z = a$ on the real axis, then

\[ \lim_{r \to 0} \int_{C_2} f(z) dz = \pi i \Res{z=a} f(z), \]

as illustrated below

\includegraphics[alt={Principal Value}]{principal-value.png}
\end{theorem}
...
\includegraphics[alt={Real and Complex Poles}]{real-and-complex-poles.png}
