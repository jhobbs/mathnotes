...
\title{Limits and Continuity of Complex Functions}
...
See first the \pagelink[Limits notes for two dimensional functions]{analysis/real-analysis/limits-of-a-function}.

\subsection{Delta Epsilon Definition}

Let $S$ be an \@{open} set containing $a$, and let $f$ be a function defined on $S$, except possibly at $a$. The \textbf{limit} of $f(z)$ as $z$ approaches $a$ is $L$, donoted as
...
\includegraphics[alt={Delta Epsilon Limit of Complex Function}, title={complex delta epsilon limit}]{complexlimit.png}
...
A \@{complex-function} $f$ is said to be \textbf{continuous} at $z = z_0$ if
...
\begin{enumerate}
\item $f$ must be defined at $z_0$;
\end{enumerate}

\begin{enumerate}
\item the limit of $f$ as $z \to z_0$ must exist;
\end{enumerate}

\begin{enumerate}
\item the numbers in 1. and 2. must be identical.
\end{enumerate}

\@{Continuous} functions can be added, subtracted, multiplied, and divided, and their results will still be continuous.

A discontinuity in a function $f$ at $z_0$ is said to be a \textbf{removable discontinuity} if we can assign a value to the function $f$ at $z_0$ that makes the function continuous.

\subsection{Branches, Branch Points and Branch Cuts}
...
This restricted range version of $\arg{z}$ is said to be a \textbf{branch} of $\arg{z}$. The half-line along which it is discontinuous is called the \textbf{branch cut} of the branch, and the point $z = 0$ at which all branch cuts meet is called the \textbf{branch point} of the multi-valued function $\arg{z}$.
