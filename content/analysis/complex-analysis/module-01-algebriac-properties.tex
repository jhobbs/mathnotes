...
\subsection{Complex Numbers - Basics}

A \textbf{pure imaginary number} is the product of a real number and a number $i$ which is defined by the relation $i^2 = -1$.

\begin{definition}[Complex Numbers]
\notation{\complexes}{\mathbb{C}}
A \textbf{complex number} is one which can be written in the form $a + bi$, where $a$ and $b$ are \@{real-numbers}. The set of all complex numbers is denoted $\complexes$.
\end{definition}

Note that the complex numbers include both the \@{real-numbers} and the imaginary numbers.

The $a$ in the complex number $z = a + bi$ is called the \textbf{real part of z;} the $b$ is called the \textbf{imaginary part of z}.

Note that the imaginary part of a complex number is itself real.

If $ z = a +bi$, then the \textbf{conjugate of z,} written as $\bar{z}$, is $\bar{z} = a - bi$.

To form the conjugate of a complex number, simply change the sign of the coefficient of $i$.

If $z = a + bi$, then the \textbf{absolute value of z} or \textbf{modulus of z}, written as $\lvert z \rvert$, is $\lvert z \rvert = \sqrt{a^2 + b^2}$.
...
\includegraphics[alt={Complex Plane}]{complex-plane.jpg}
...
\includegraphics[alt={Complex Polar and Rectangular Coordinates}]{complex-polar-and-rectangular.jpg}
...
and call it the \textbf{rectangular form of z}, we can replace $x$ and $y$ with their polar equivalents to get:

\[ z = r\cos\theta + ir\sin\theta = r(cos\theta + i\sin\theta) \]

which is called the \textbf{polar form of z}.

The polar angle $\theta$ in the figure above is called the \textbf{principal argument of z}, written as $\Arg z$, and is defined to be an angle in $(-\pi, \pi]$ satisfying the two equalities:
...
Writing it as $\text{arg}z$ means that instead of taking the \textbf{principal value} of $\theta$, we have a multivalued function giving all possible angles satisfying those two equalities:

\[ \text{arg} z = \Arg z \pm 2n\pi,~n = 1,2,3 \]
...
We can also write a complex number in \textbf{exponential form}, by taking advantage of Euler's formula:
...
We can use this property to easily compute powers of complex numbers, which is sometimes called \textbf{DeMoivre's theorem}:

\begin{theorem}[de Moivre's Theorem]\label{de-moivres-}
\[ z^n = r^n ( \cos{n\theta} + i \sin{n \theta} ) \]
\end{theorem}

\begin{theorem}[nth root of z]\label{nth-root-of-z}
...
By definition, the \textbf{principal square root} of $Z$ is the square root with the non-negative real part.

\subsection{Complex Quadratics}
...
\subsection{Exponential, Trigonometric and Hyperbolic Functions of Complex Numbers}
...
and are called, respectively, the \textbf{hyperbolic sine}, \textbf{hyperbolic cosine}, and \textbf{hyperbolic tangent} of $z$.

\subsection{Proof that imaginary roots of polynomials with real coefficients occur in pairs}

(\emph{jmh} my notes, the answers to exercises 18.6 through 18.9.)
