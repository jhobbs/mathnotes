...
A complex-valued function $f$ of a complex variable $z$ is a rule that assigns a complex number $f(z)$ to each complex number $z$ in a set $S$. We call such a \dref{function} a \textbf{complex function.}
\end{definition}

The set of complex numbers on which a function operates is called the \textbf{domain of definition.}. It can be a domain in the sense described in \pagelink[Regions of the Complex Plane]{analysis/complex-analysis/module-02-regions-of-the-complex-plane}, but it doesn't have to be. If the domain of definition isn't explicitly specified, it is assumed to be the entire complex plane.

We often use the format

\[ w = f(z) \]

to denote that $f$ maps values of $z$ in the domain to values of $w$ in the \textbf{range}, which is the set of all values the function produces from input values in the domain of definition.
...
\subsection{Some Geometricly Interesting Functions}
...
is called a \textbf{reciprocation}. Again, writing it in the exponential form is more enlightening:
...
Peter Francis has an amazing \href{https://peterefrancis.com/complex-function-plot/index.html}{tool for graphing complex functions}.

\subsection{Bilinear Mappings}

Another important class of functions is called \textbf{bilinear} or \textbf{Mobius} mappings. They are of the form
...
\subsection{Real and Imaginary Parts of Complex Functions}
...
\subsection{Finding the Image of a Curve}

The \textbf{image} in the $w$-plane of a curve $C$ in the $z$-plane is denoted $C'$ and can be found by two distinct procedures.

Suppose that $g(x,y) = 0$ is the equation of a curve $C$ in the $z$-plane.

\textbf{Procedure 1:} If it is convenient to invert $w = f(z)$ and find $z$ in terms of $w$, $z = h(w)$, then we effectively have $x$ and $y$ in terms of $u$ and $v$. We can then substitute these values into $g(x,y) = 0$ to give the equation $g[x(u,v), y(u,v)] = 0$ for $C'$.

\textbf{Procedure 2:} If it is inconvenient to find $z$ in terms of $w$, we can represent $C$ parametrically, $x = x(t)$ and $y = y(t)$. If we substitute these into $u = u(x,y)$ and $v = v(x,y)$, we obtain parametric equations $u = u[x(t), y(t)], v = v[x(t), y(t)]$ for $C'$.

\subsection{The Point at Infinity ##}

There isn't just one point at infinity. In fact, if you go infinitely far away from the origin in any direction, you will arrive at "the point at infinity". Depending on the convention of a text, the complex plane can be considered to exclude the point at infinity by default, in which case the complex plane plus the point at infinity is called the \textbf{extended complex plane}, or the complex plane is considerd to include the point at infinity by default, in which case the complex plane without the point at infinity is referred to as the \textbf{finite complex plane}.
