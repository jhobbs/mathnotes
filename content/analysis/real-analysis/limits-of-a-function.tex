...
\section{Limits of a Function}

\subsection{Delta Epsilon Definition of a Function on an Interval}

Let $I$ be an open interval containing $a$, and let $f$ be a function defined on $I$, except possibly at $a$. The \textbf{limit} of $f(x)$ as $x$ approaches $a$ is $L$, donoted as
...
\includegraphics[alt={Delta Epsilon Limit}, title={two dimensional delta epsilon limit}]{2dlimit.png}
...
\subsection{Limits of a Function on a Set of Real Numbers}
...
Let $A$ be a set of \@{real-numbers} and $f(x)$ be a function $f : A \to \reals.$ The \textbf{limit} of $f(x)$ as $x$ approaches $a$ is $L,$ written 
...
\subsection{Limits of Function Rules}

Suppose $\lim_{x \to a}{f(x)} = L, \lim_{x \to a}{g(x)} = M$ and $c \in \reals.$ Then

\[ \begin{aligned} \lim_{x \to a}{f(x) + cg(x)} & = L + cM, \\
...
Let $A$ be a set of \@{real-numbers} containing $a$ and $f(x)$ be a function $ : A \to \reals.$ The limit of $f(x)$ as $x$ approaches $a$ \textbf{from the right} is $L,$ written 
...
Let $A$ be a set of \@{real-numbers} containing $a$ and $f(x)$ be a function $f : A \to \reals.$ The limit of $f(x)$ as $x$ approaches $a$ \textbf{from the left} is $L,$ written 
...
\emph{Theorem}: Let $A$ be a set of \@{real-numbers} containing $a$ and $f(x)$ be $f : A \to \reals,$ and assume that for every $\delta > 0$ there are numbers $x \in A$ satisfying $0 < \| x - a \| \delta.$ Then $\lim_{x \to a}{f(x)} = L$ if and only if $\lim_{x \to a^-}{f(x)} = \lim_{x \to a^+}{f(x)} = L.$

\subsection{Limits Involving Infinity}
Let $A$ be a set of \@{real-numbers} and $f(x)$ a function from $A$ into $\reals.$ We say that the limit of $f(x)$ as $x$ approaches infinity is $L,$ written as

\[ \lim_{x \to \infty}{f(x)} = L, \]

if for every $\epsilon > 0$ there exists a real number $M$ such that $\|f(x) - L\| < \epsilon$ if $x \geq M.$ 

Let $A$ be a set of \@{real-numbers} and $f(x)$ a function from $A$ into $\reals.$ We say that the limit of $f(x)$ as $x$ approaches negative infinity is $L,$ written as
...
\subsubsection{Factoring a root out of a numerator}
