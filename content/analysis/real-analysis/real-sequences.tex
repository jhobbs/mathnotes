...
A \textbf{real sequence} of numbers if a \@{function} $f$ from $\naturals$ to $\reals$.
\end{definition}

\begin{note}\label{real-sequence-notation}
...
We say that a \@{sequence} has a limit $L$ if we can make terms arbitrarily close to $L$ by taking $n$ to be sufficiently large. More precisely, the \textbf{limit} of a sequence $a_n$ as $n$ approaches infinity is $L$ ($\lim_{n \to \infty} a_n = L$) if for every $\epsilon > 0$ there exists an $N \in \naturals$ such that $\|a_n -L\| < \epsilon$ whenever $n \geq N$.

If a sequence's limit exists and is finite, we say the sequence \@{converges} to the limit and is a \textbf{convergent sequence.}

Example: The \@[limit]{limit-sequence} of the sequence $a_n = \frac{3n}{n+1}$ is $3$. \emph{Proof:} We need to show that given $\epsilon > 0$, there is some $N \in \naturals$ where if $n > N$, $\|\frac{3n}{n+1} - 3\| < \epsilon$. First, note that
...
\subsection{Uniqueness of the Limit of a Sequence}

Note that the limit of a sequence is unique, that is, if $\lim_{n \to \infty} = L$ and $\lim_{n \to \infty} = M,$ then $L = M$. \emph{Proof:}. Suppose $L > M$. Then $\frac{L - M}{2} > 0,$ and for some $N_1, N_2 \in \naturals$ we have
...
which can be \pagelink[rewritten]{analysis/real-analysis/real-numbers} as
...
A sequence is said to be a \textbf{bounded sequence} if there exists a \@{real-number} $M$ such that $\|a_n\| \leq M$ for all natural numbers $n$. In other words, $a_n$ is never further from $0$ than $\pm M$. If $a_n$ is less than or equal to some real number for all $n$, then $a_n$ is said to be \textbf{bounded above}, and if $a_n$ is greater than or equal to some real number for all $n$, it is said to be \textbf{bounded below.}

\subsection{Divergent Sequences}
In general, any sequence that does not \@{converge} is said to \textbf{diverge} and is a \textbf{divergent sequence}.

The real sequence $a_n$ is said to \textbf{diverge to infinity} if for every \@{real-number} $M$, there exists a \@{natural-number} $N$ such that $a_n > M$ whenever $n \geq N.$ In this case, we write $\lim_{n->\infty}{a_n} = \infty$. This means that for any given real number, not matter how large, we can find an $n$ that makes $a_n > M$.  

The real sequence $a_n$ is said to \textbf{diverge to minus infinity} if for every real number $M$, there exists a natural number $N$ such that $a_n < M$ whenever $n \geq N.$ In this case, we write $\lim_{n->\infty}{a_n} = -\infty$. This means that for any given real number, not matter how small, we can find an $n$ that makes $a_n < M$. 
...
The real sequence $a_n$ is said to be \textbf{increasing} if $a_n \leq a_{n+1}$ for all natural $n$, and is said to be \textbf{decreasing} if $a_n \geq a_{n+1}$ for all natural $n$.

A real sequence is $a_n$ is said to be \textbf{monotone} if it is either increasing or decreasining.

The \textbf{monotone convergence theorem} says that if a sequence is monotone and \@[bounded]{bounded-sequence}, then it converges. \emph{Proof}: Suppose that the sequence $a_n$ is bounded and increasing. Then, by the Completeness axiom, the set $\{a_n | n \in \naturals \}$ has a \@{least-upper-bound}; let's call it $L.$ Then, for any positive $\epsilon$, $L - \epsilon$ can't be an \@{upper-bound} of the set, since $L - \epsilon < L$. Therefore, for some $N \in \naturals$, $L - \epsilon < a_N$, and since $a_n$ increases monotonically, for all $n \geq N$, we have $L - \epsilon < a_n < L + \epsilon$, which implies $|a_n - L| < \epsilon,$ that is, that $a_n$ converges to $L$. $\square$

\subsection{Subsequences}

The sequence $a_{f(n)}$ is called a \textbf{subsequence} of the sequence $a_n$ if $f : \naturals \to \naturals$ is a strictly increasing function. Another way to put this is that if $a_n$ is a sequence, if we construct $a'_n$ by removing $0$ or more elements from $a_n$ and leaving the remaining elements in the same order, then $a'_n$ is a \@{subsequence} of $a_n.$

The \textbf{Limit of a Subsequence Theorem} says that if $a_n$ is a convergent sequence with limit $L$, then any subsequence $a_{f(n)}$ of $a_n$ will also converge to $L.$ \emph{Proof}: Suppose $a_n$ converges to $L.$ Let $\epsilon > 0.$ Then for some $N \in \naturals$, for $n \geq N,$ $\|a_n - L\| < \epsilon$. Since $f(n)$ is strictly increasing, $f(n) \geq n$ for all $n,$ so if $n \geq N,$ then $f(n) \geq n \geq N,$ and $\|a_{f(n)} - L\| < \epsilon.$ $\square$
...
\subsection{Bolzano-Weirstrass Theorem}

The \textbf{Bolzano-Weirstrass Theorem} states that if a sequence of \@{real-numbers} is \@[bounded]{bounded-sequence}, then it has a \@{convergent} \@{subsequence}.

\subsection{Cauchy Sequences}

A sequence is said to be \textbf{Cauchy} if for every $\epsilon > 0$, there exists a \@{natural} $N$ for which $\|a_m - a_n\| < \epsilon$ whenever $m, n > N.$  In other words, as we go further in the sequence, the terms get arbitrarily closer together.
