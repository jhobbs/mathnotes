...
A sequence $\{p_n\}$ in a \@{metric-space} $X$ is said to \textbf{converge} if there is a point $p \in X$ with the following property: For every $\epsilon > 0,$ there is an \@{integer} $N$ such that $n \geq N$ implies that $d(p_n, p) < \epsilon.$
\end{definition}

\begin{definition}[Convergent]
If a \@{sequence} \@{converges}, it is said to be a \textbf{convergent} \@{sequence}.
\end{definition}

\begin{definition}[Limit (Sequence)]\label{limit-sequence}
If a \@{sequence} $\{p_n\}$ converges to $p,$ we say that $p$ is the \textbf{limit} of $\{p_n\},$ denoted as:

\[ \lim_{n \to \infty} p_n = p. \]
\end{definition}

\begin{definition}[Diverge]
If a \@{sequence} $\{p_n\}$ does not \@{converge}, it is said to \textbf{diverge.}
\end{definition}

\begin{definition}[Range (sequence)]
The set of all points $p_n$ of a sequence $\{p_n\} (n = 1, 2, 3, \dots)$ is the \textbf{\@{range}} of $\{p_n\}.$
\end{definition}
...
The \@{sequence} $\{p_n\}$ is said to be \textbf{bounded} if its \@{range-sequence} is \@{bounded}.
\end{definition}
...
Given a \@{sequence} $\{p_n\},$ consider a \@{sequence} $\{n_k\}$ of positive integers, such that $n_1 < n_2 < n_3 < \cdots.$ Then the \@{sequence} $\{p_{n_i}\}$ is called a \textbf{subsequence} of $\{p_n\}.$
\end{definition}

\begin{definition}[Subsequential limit]
If a subsequence $\{p_{n_i}\}$ of $\{p_n\}$ \@{converges}, its \@{limit-sequence} is called a \textbf{subsequential limit} of $\{p_n\}.$
\end{definition}
...
\bal d(q, p_{n_i}) & \leq d(q, x) + d(x, p_{n_i}) \\
...
A \@{sequence} $\{p_n\}$ in a \@{metric-space} $X$ is said to be a \textbf{Cauchy sequence} if for every $\epsilon > 0$ there is an \@{integer} $N$ such that $d(p_n, p_m) < \epsilon$ is $n \geq N$ and $m \geq N.$
\end{definition}

\begin{definition}[Diameter]
Let $E$ be a nonempty \@{subset} of a \@{metric-space} $X,$ and let $S$ be the \@{set} of all \@{real-numbers} of the form $d(p,q),$ with $p, q \in E.$ The \@{supremum} of $S$ is called the \textbf{diameter} of $E.$
\end{definition}
