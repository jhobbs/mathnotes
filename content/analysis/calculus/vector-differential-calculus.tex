...
Let $\vec{f}$ be a \@{function} that maps an \@{open} \@{set} $E \subset R^n$ into $R^m.$ Let $\{\vec{e}_1, \dots, \vec{e}_n\}$ and $\{\vec{u}_1, \dots, \vec{u}_n\}$ be the standard bases of $R^n$ and $R^m.$ The \textbf{components} of $\vec{f}$ are the real \@{functions} $f_1, \dots, f_m$ defined by
