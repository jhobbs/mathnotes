...
Let $P$ be any \@{point} in a \@{domain-of-definition}. Then we define a \textbf{vector function} $\vec{v}$ whose \@{values} are \@{vectors}, that is,

\[ \vec{v} = \vec{v}(P) = [v_1(P), v_2(P), v_3(P)] \]

that depends on points $P$ in space. A \@{vector-function} depends only on the point $P,$ not on the coordinate system chosen to represent its \@[components]{component-function}.
\end{definition}

\begin{definition}[Vector Field]
We say that a \@{vector-function} defines a \textbf{vector field} in a \@{domain-of-definition}.
\end{definition}

\begin{definition}[Scalar Function]\synonyms{"scalar field"}
Let $P$ be any \@{point} in a \@{domain-of-definition}. Then we define a \textbf{scalar function} $f,$ whose \@{values} are \@{scalars}, that is,
...
Let $\vec{f}$ be a \@{function} that maps an \@{open} \@{set} $E \subset R^n$ into $R^m.$ Let $\{\vec{e}_1, \dots, \vec{e}_n\}$ and $\{\vec{u}_1, \dots, \vec{u}_n\}$ be the standard bases of $R^n$ and $R^m.$ The \textbf{component functions} of $\vec{f}$ are the real \@{functions} $f_1, \dots, f_m$ defined by
...
For a space curve given parametrically by $\vec{r}(t),$ the \textbf{tangent vector} (or specifically, the \textbf{unit tangent vector}) at the point $\vec{r}(t)$ is the \@{unit-vector} defined by:
...
The derivative defined above in \@{differentiable} is called the \textbf{total derivative} of $\vec{f}$ at $\vec{f},$ or the \textbf{differential} of $\vec{f}$ at $\vec{x}.$
\end{definition}
...
A \@{differentiable} \@{function} $\vec{f}$ of an \@{open} \@{set} $E \subset R^n$ into $R^m$ is said to be \textbf{continuously differentiable} in $E$ if $\vec{f}'$ is a \@{continuous} function of $E$ into $L(R^n, R^m).$ More explicitly, it is required that to every $\vec{x} \in E$ and to every $\epsilon > 0$ corresponds a $\delta > 0$ such that

\[ \lVert\vec{f}'(\vec{y}) - \vec{f}'(\vec{x})\rVert < \epsilon \]
...
In Euclidean space $\reals^n$ with coordinates $(x_1, \dots, x_n)$ and standard basis $(\vec{e}_1, \dots, \vec{e}_n),$ \textbf{del} is a vector operator whose $x_1, \dots, x_n$ \@{components} are the \@{partial-derivative} operators $\frac{\partial}{\partial x_1}, \dots, \frac{\partial}{\partial x_n}; $ that is
...
is called the \textbf{directional derivative} of $f$ at $\vec{x}$ in the \@{direction} of the \@{unit-vector} $u$ and is denoted as $D_{\vec{u}}f(\vec{x}).$
\end{definition}
...
Let $S$ be a surface represented by $f(x, y, z) = c,$ where $c$ is constant and $f$ is \@{differentiable}. Such a surface is called a \textbf{level surface} of $f,$ and for different $c,$ we get different level surfaces.
\end{definition}

\begin{definition}[Tangent Plane]
If $S$ is a \@{level-surface} of a function $f,$ and $P$ is a \@{point} of $S,$ then the set of all \@{tangent-vectors} of all curves passing through $P$ will generally form a plane, called the \textbf{tangent plane} of $S$ at $P.$
\end{definition}

\begin{definition}[Surface Normal]
Given a \@{tangent-plane} to a surface $S$ at $P,$ the normal to this plane (the straight line through $P$ \@{perpendicular} to the \@{tangent-plane}) is called the \textbf{surface normal} to $S$ at $P.$
\end{definition}

\begin{definition}[Surface Normal Vector]
A \@{vector} in the \@{direction} of the \@{surface-normal} of a surface $S$ at point $P$ is called a \textbf{surface normal vector} of $S$ at $P.$
\end{definition}
...
A \textbf{potential function} is a \@{scalar-function} whose \@{gradient} is a \@{vector-field}. They allow representing certain \@{vector-fields} in a simpler, more fundamental form.
...
The \textbf{normal derivative} is the \@{directional-derivative} in the \@{direction} of the \@{normal-vector}.
\end{definition}
...
For a \@{vector-field} $\vec{F} = (F_1, F_2, F_3)$ defined in three-dimensional space $\reals^3$, the \textbf{divergence} is defined as
...
Let $\vec{v}$ be the velocity \@{vector} of of the motion of particles in a fluid. If $\div{\vec{v}} = 0,$ then the fluid has constant density and is said to be \textbf{incompressible.}
\end{definition}

\subsection{Curl}

\begin{definition}[Curl]
For a \@{vector-field} $\vec{F} = (F_1, F_2, F_3)$ defined in three-dimensional space $\reals^3$, with each \@{component} function $F_i$ depending on the variables $x$, $y$, and $z$, the curl of $\vec{F}$ is defined as
...
If the \@{curl} of a \@{vector-field} is $\vec{0},$ i.e. if $\curl{\vec{v}} = 0,$ the field is said to be \textbf{irrotational.}
\end{definition}
...
Suppose $f ~ : ~ \reals^n \to \reals$ is a \@{function} taking as input a \@{vector} $\vec{x} \in \reals^n$ and outputting a \@{scalar} $f(\vec{x}) \in \reals.$ If all second-order \@{partial-derivatives} of $f$ exist, then the \textbf{Hessian matrix} $\vec{H}$ of $f$ is a square $n \times n$ \@{matrix}, usually defined and arranged as 
...
For a \@{function} $f ~ : ~ \reals^3 \to \reals,$ this is
...
Let $\vec{f} ~ : ~ \reals^n \to \reals^m$ be a \@{function} such that each of its first-order \@{partial-derivatives} exists on $\reals^n.$ This \@{function} takes a point $\vec{x} = (x_1, \dots, x_n) \in \reals^n$ as input and produces the \@{vector} $\vec{f}(\vec{x}) = (f_1(\vec{x}), \dots, f_m(\vec{x})) \in \reals^m$ as output. Then the \textbf{Jacobian matrix} of $\vec{f},$ denoted $\vec{J}_{\vec{f}},$ is the $m \times n$ \@{matrix} whose $(i,j)$ \@{entry} is $\frac{\partial f_i}{\partial x_j};$ explicitly
...
At a point $x_0 \in \reals^n,$ the \@{directional-derivative} of $F$ in the \@{direction} of $\vec{v} \in \reals^n$ is
...
\begin{itemize}
\item The \@{rows} are the \@{transposes} of \@{gradients} of each \@{component} of $\vec{F}.$
\end{itemize}

\begin{itemize}
\item The \@{columns} tell how each \@{coordinate} \@{direction} affects all \@{components} of $\vec{F}.$
\end{itemize}
...
The \textbf{trace} measures how much the \@{linear-transformation} represented by $\vec{J}$ stretches space along its coordinate axes - it's the infinitesimal "net expansion rate."
...
\begin{itemize}
\item The \@{direction} of $\vec{\omega}$ is the axis of rotation (via the right-hand rule).
\end{itemize}

\begin{itemize}
\item The \@{magnitude} $\lVert\omega\rVert$ is the local \@{angular-velocity} (half the \@{curl} \@{magnitude}.)
\end{itemize}

More visually, if we take a small \@{ball} of \@{points} around $\vec{x_0},$

\begin{itemize}
\item The symmetric part $\vec{S}$ of $\vec{J}$ turns the ball into an \@{ellipsoid} (by stretching/shearing it).
\end{itemize}
...
\begin{itemize}
\item Its spin rate is $\frac{1}{2} \lVert \nabla \times \vec{F} \rVert.$ 
\end{itemize}
