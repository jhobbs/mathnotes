...
For a \@{vector-field} $\vec{F} = (F_1, F_2, F_3)$ defined in three-dimensional space $\mathbb{R}^3$, the \textbf{divergence} is defined as
...
Suppose $f ~ : ~ \mathbb{R}^n \to \mathbb{R}$ is a \@{function} taking as input a \@{vector} $\vec{x} \in \mathbb{R}^n$ and outputting a \@{scalar} $f(\vec{x}) \in \mathbb{R}.$ If all second-order \@{partial-derivatives} of $f$ exist, then the \textbf{Hessian matrix} $\vec{H}$ of $f$ is a square $n \times n$ \@{matrix}, usually defined and arranged as 
