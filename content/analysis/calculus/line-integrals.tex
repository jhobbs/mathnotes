...
\title{Arc Length and Line Integrals}
...
\subsection{Lengths of Parametric Curves}

\begin{definition}[smooth]
A function with a \dref{continuous} first derivative is said to be \textbf{smooth}.

For a parametrically defined function where $x = f(t), y = g(t)$, both $f'$ and $g'$ must be \dref{continuous}.
\end{definition}
...
\includegraphics[alt={Arc Length Differential}, title={arc length differential}]{ds.png}
...
\begin{theorem}\label{arc-length-in-plane}
...
\begin{theorem}\label{line-integral-over-a-plane-curve}
...
\begin{theorem}\label{all-parameterizations-of-curve-have-same-line-integral}
...
\begin{definition}[Path Independent]\synonyms{"independent of path"}
We say that line integral 4.7 is \textbf{independent of path} in a domain $D$ if for any two points $A$ and $B$ in $D$, the value of the line integral is the same for all piecewise smooth curves in $D$ from $A$ to $B$.
\end{definition}
...
\begin{enumerate}
\item Show that $P\mathbf{\hat{i}} + Q\mathbf{\hat{j}}$ is the gradient of some function $\phi(x,y)$ at every point of $D$.
\end{enumerate}

\begin{enumerate}
\item If $D$ is simply-connected (every closed curve in $D$ contains in its interior only points of $D$), show that $\partial Q/\partial x = \partial P / \partial y$.
\end{enumerate}
\end{remark}

\begin{theorem}\label{fundamental-theorem-of-line-integrals}
...
\begin{theorem}\label{closed-path-of-path-independent-integral-is-zero}
