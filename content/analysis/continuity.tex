...
Suppose $X$ and $Y$ are \@{metric-spaces}, $E \subset X, p \in E,$ and $f : E \to Y.$ Then $f$ is said to be \textbf{continuous at $p$} if for every $\epsilon > 0$ there exists a $\delta > 0$ such that

\[ d_Y(f(x), f(p)) < \epsilon \]

for all points $x \in E$ for which $d_X(x, p) < \delta.$

If $f$ is \textbf{continuous} at every \@{point} of $E,$ then $f$ is said to be \textbf{continuous on $E$.}
\end{definition}
...
The \@{function} $h$ is called the \textbf{composition} or the \textbf{composite} of $f$ and $g.$ The notation
...
\begin{corollary}\label{mapping-continuous-iff-inverse-images-of-closed-sets-are-closed}
A \@{mapping} $f$ of a \@{metric-space} $X$ into a \@{metric-space} $Y$ is \@{continuous} on $X$ if and only if $f^{-1}(C)$ is \@{closed} in $X$ for every \@{closed} set $C$ in $Y.$
\end{corollary}

\begin{proof}
First, note that for any $E \subset Y,$ we have $f^{-1}(E^c) = f^{-1}(E)^c,$ since $x \in f^{-1}(E^c)$ exactly when $f(x) \notin E,$ which is exactly when $x \notin f^{-1}(E).$

Now, as $V$ ranges over the \@{open} \@{subsets} of $Y,$ its \@[complement]{complement-of-a-set} $V^c$ ranges over the \@{closed} subsets of $Y,$ by \@{open-iff-complement-closed}. So, by \@{mapping-continuous-iff-inverse-images-of-open-sets-are-open}, $f$ is continuous on $X$ if and only if $f^{-1}(C^c)$ is \@{open} in $X$ for every closed set $C$ in $Y.$ By the observation above, $f^{-1}(C^c) = f^{-1}(C)^c,$ and this is open in $X$ if and only if $f^{-1}(C)$ is closed in $X,$ again by \@{open-iff-complement-closed}.
\end{proof}
