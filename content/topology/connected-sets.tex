...
\begin{note}\label{connected-sets-reference-note}
...
Two subsets $A$ and $B$ of a metric space $X$ are said to be \textbf{separated} if both $A \cap \closure{B}$ and $\closure{A} \cap B$ are empty, i.e., if no point of $A$ lies in the closure of $B$ and no point of $B$ lies in the closure of $A.$
\end{definition}

\begin{definition}[Connected]
If $X$ is a metric space, a set $E \subset X$ is said to be \textbf{connected} if $E$ is not a union of two nonempty separated sets.
\end{definition}

We have a different - less general, but compatible - definition of connected from complex analysis:

\dref{embed}\{connected-complex\}

This theorem helps connect the two definitions.

\begin{theorem}\label{connected-sets-in-r1-are-intervals}
...
By \dref{sup-is-in-closure-of-bounded-nonempty-set-of-reals}, $z \in \closure{A},$ and because $A$ and $B$ are separated, $z \notin B.$ Therefore $x \leq z < y.$
