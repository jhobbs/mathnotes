...
\begin{note}\label{perfect-sets-reference-note}
...
\dref{embed}\{perfect-set\}

\begin{theorem}\label{non-empty-perfect-sets-in-rk-are-uncountable}
Let $P$ be a nonempty perfect set in $R^k.$ Then $P$ is uncountable.
\end{theorem}

\begin{proof}
We know that $P$ is infinite, because by definition, \dref[all points in perfect sets are limit points]{perfect-set}, and \dref[only infinite sets have limit points]{only-infinite-sets-have-limit-points}.
...
As a base step, let $V_1$ be any neighborhood of $x_1;$ let $V_1 = \{ y \in R^k | ~ |y - x_1| < r \}$ (note: subsequent $V_{n+1}$ aren't required to be neighborhoods of $x_{n+1}.$) Then the \dref{closure} $\closure{V_1}$ of $V_1$ is $\closure{V_1} =  \{ y \in R^k | ~ |y - x_1| \leq r \}.$
...
For each $n$, let $K_n = \closure{V_n} \cap P.$ Since $\closure{V_n}$ is closed and bounded, $\closure{V_n}$ \dref[is compact]{heine-borel}. Since $x_n \notin \closure{V_{n+1}},$ no point of $P$ lies in $\bigcap_{n=1}^\infty K_n.$ Since $K_n \subset P,$ this implies that $\bigcap_{n=1}^\infty K_n$ is empty. But, each $K_n$ is nonempty, by (iii), and $K_{n+1} \subset K$, by (i). But the \dref[intersection of nonempty compact nested sets is nonempty]{intersection-of-nonempty-nested-compact-sets-is-nonempty}, so we have a contradiction, so our provisional assumption that $P$ is countable must be incorrect. Therefore, $P$ is uncountable.
\end{proof}

\begin{corollary}\label{every-interval-is-uncountable}
...
\begin{theorem}\label{euclidean-space-is-separable}
$R^k$ is separable.
\end{theorem}
...
We know that \dref[the rationals are countable]{rationals-are-countable}, and because \dref[$n$-tuples of countable elements are countable]{n-tuples-of-countable-elements-are-countable}, $Q^k$ is countable.

To show $Q^k$ is dense, consider an arbitrary point $p$ in $R^k.$ Now, let $\epsilon > 0.$ Since the \dref[the rationals are dense in the reals]{rationals-are-dense-in-reals}, we can pick a $q \in Q^k$ with $d(p, q) < \epsilon,$ by picking rational approximations of the coordinates of $p$ and forming $q$ such that $q$ is within $\epsilon$ of $p,$ i.e.
...
A collection ${V_\alpha}$ of open subsets of $X$ is said to be a \textbf{base} for $X$ if the following is true: For every $x \in X$ and every open set $G \subset X$ such that $x \in G,$ we have $x \in V_\alpha \subset G$ for some $\alpha.$ In other words, every open set in $X$ is the union of a subcollection of ${V_\alpha}.$
\end{definition}

\begin{theorem}\label{every-separable-metric-space-has-a-countable-base}
Every separable metric space has a countable base.
\end{theorem}

\begin{proof}
Let $X$ be a \dref{separable} \dref{metric-space}, and let $x \in G \subset X,$ with $G$ \dref{open-set}. Now, because $X$ is separable, it by definition has a \dref{countable} \dref{dense} subset $E.$ If $x \in E,$ we can pick a rational $\delta > 0$ and let $N_{\delta}(x)$ be a \dref{neighborhood} such that $N_{\delta}(x) \subset G$ ($\delta$ must also be small enough such that this neighborhood is within $G,$ which is possible because $x$ is an \dref{interior-point} of $G$ and we can \dref[pick a rational as close to any real as we'd like]{rationals-are-dense-in-reals}.) Now we have that $x \in N_{\delta}(x) \subset G,$ and since there are countably many $x \in E$ and countably many neighborhoods with rational radius around each $x \in E,$ there are countably many such neighborhoods in $G.$ On the other hand, if $x \notin E,$ then $x$ is a \dref{limit-point} of $E,$ and thus there is $p \in E$ as close as we'd like to $x.$ Pick $p \in E$ such that $d(p,x) < \delta$ for some rational $\delta$ such that $x \subset N_{\delta}(p) \subset G.$ Again, since there are countably many such $p \in E,$ with countably many neighborhoods of rational radius each, there are countably many such neighborhoods in $G.$

Now, if we let $G$ be the union of all open sets $G_\beta \subset X,$ then $G$ \dref[is open]{union-and-intersection-of-open-and-closed-sets}, and let ${V_\alpha}$ be the union of all the $N_r(q), q \in G \cap E,$ with $r \in Q,$ then every $x \in G$ is in some ${V_\alpha},$ and there are countably many ${V_\alpha},$ so ${V_\alpha}$ is a countable \dref{base} for $X.$
\end{proof}

\begin{theorem}\label{every-metric-space-where-every-infinite-subset-has-a-limit-point-is-separable}
Let $X$ be a metric space in which every infinite subset has a limit point. Then $X$ is \dref{separable}.
\end{theorem}

\begin{proof}
Let $\delta > 0,$ and pick $x_1 \in X.$ Now, continue picking $x_{j+1}$ such that $d(x_i, x_{j+1}) \geq \delta$ i.e. so that each new point is at least $\delta$ away from each existing point. Suppose this process continues infinitely; then the points $\{x_i\}$ are an infinite subset of $X,$ and thus must have a limit point $p.$ Now, because \dref[every neighborhood of $p$ must contain infinitely many points in ${x_i}$]{neighborhood-of-limit-point-contains-infinitely-many-points}, we can let $r = \delta / 2,$ $N_r{p}$ be a neighborhood of $p,$ and pick $x, y \in \{x_i\}, N_r{p}.$ But, by the triangle inequality
...
\begin{theorem}\label{compact-metric-space-has-countable-base}
...
A point $p$ in a metric space $X$ is said to be a \textbf{condensation point} of a set $E \subset X$ if every neighborhood of $p$ contains uncountably many points of $E.$
\end{definition}

\begin{example}\label{condensation-point-example}
Let $p \in \mathbb{R}, \epsilon > 0$ and let $E = (p, p+\epsilon).$ Then, for any $0 < \delta < \epsilon$ let $N_{\delta}(p)$ be an open ball around $p.$ Then, $N_{\delta}(p) \cap E = (p, p + \delta)$ is an open interval in $E,$ and \dref[is therefore uncountable]{every-interval-is-uncountable}.
\end{example}

\begin{theorem}\label{condensation-points-of-an-uncountable-subset-of-rk-are-perfect}
...
\begin{proof}
Let $\{V_n\}$ be a countable base of $R^k$ (see \dref{euclidean-space-is-separable} and \dref{every-separable-metric-space-has-a-countable-base},) and let $W$ be the union of those $V_n$ for which $E \cap V_n$ is at most countable. We will show that $P = W^c.$
...
\begin{theorem}\label{cantor-bendixson-theorem}
...
Suppose that $E$ is uncountable. Note that the proof of \dref{condensation-points-of-an-uncountable-subset-of-rk-are-perfect} only uses the property that $R^k$ is a separable metric space, and it therefore generalizes to any separable metric space. Thus $E$ is the union of a perfect set - its condensation points, $P$, and a set that is at most countable, $E \setminus P.$
\end{proof}

\begin{corollary}\label{countable-closed-set-has-isolated-points}
Every \dref{countable} \dref{closed} \dref{set} set in $R^k$ has \dref{isolated-points}.
\end{corollary}

\begin{proof}
Let $E$ be a countable closed set in $R^k.$ Suppose for contradiction that $E$ has no isolated points. Then every point of $E$ is a limit point of $E,$ and thus $E$ is \dref{perfect-set}. But, \dref[every nonempty perfect set in $R^k$ is uncountable]{non-empty-perfect-sets-in-rk-are-uncountable}, a contradiction. Thus, our assumption that $E$ has no isolated points is incorrect, and $E$ must contain isolated points.
\end{proof}

\begin{theorem}\label{open-set-in-r1-is-countable-union-of-disjoint-segments}
Every open set in $R^1$ is the union of an at most \dref{countable} union of disjoint segments.
\end{theorem}

\begin{proof}
Let $E$ be an open set in $R^1.$ Let $x \in E.$ Let $a = \inf \{ y : (y, x] \subset E \}$ and $b = \sup \{ y : [x, y) \subset E \}.$ Then, $a < x < b,$ because $E$ is open and thus $x$ is an \dref{interior-point} of $E$ and there is some \dref{neighborhood} around $x$ that is entirely within $E.$ Let $I(x) = (a, b).$ By construction, $I(x)$ is \dref{connected}. For all $u \in I(x),$ $I(u)$ must have the same end points as $I(x),$ since if it extended beyond, our construction of $I(x)$ would be contradicted. Also note that for all $v \in E \setminus I(x),$ $I(v) \cap I(x) = \emptyset,$ for if they intersected, they would form an \dref[open interval]{union-and-intersection-of-open-and-closed-sets}. Thus, each $I(x), x \in E$ is either disjoint from all others or identical to some other $I(y), y \in E,$ and $E$ is the union of all such unique $I.$

Now, in each $I,$ we can \dref[pick a rational number]{rationals-are-dense-in-reals}. Because \dref[the rationals are countable]{rationals-are-countable}, we have at most countably many unique $I,$ and $E$ is their union.
\end{proof}

\begin{theorem}[Special case of Blaire's theorem]\label{baire-category-theorem-special-case}
...
\begin{proof}
First, note that since $R^k = \bigcup_{n=1}^{\infty}F_n,$ every point in $R^k$ is in some $F_n,$ and so $\bigcap_{n=1}^\infty F_n^c$ \dref[must be empty]{complement-of-union-is-intersection-of-complements}.
...
As a base step, let $x_1$ be some point in $R^k,$ and let $B_1$ be an open ball around $x_1.$ Since the interior of $F_1$ is empty, $B_1 \cap F_1^c$ is not empty, and \dref[is open]{union-and-intersection-of-open-and-closed-sets}.
