...
\begin{proof}
Let $K$ be a \@{compact} \@{metric-space}. Fix $n \in \naturals$ (any natural number will do,) then, consider the \@{open-cover} $\{G_\alpha\}_n$ of \@{balls} of radius $1/n$ centered at $x \in K.$ Now, because $K$ is compact, some finite $\{V_\alpha\}_n \subset \{G_\alpha\}_n$ covers $K.$
...
Let $p \in \reals, \epsilon > 0$ and let $E = (p, p+\epsilon).$ Then, for any $0 < \delta < \epsilon$ let $N_{\delta}(p)$ be an open \@{ball} around $p.$ Then, $N_{\delta}(p) \cap E = (p, p + \delta)$ is an open interval in $E,$ and \@[is therefore uncountable]{every-interval-is-uncountable}.
\end{example}
