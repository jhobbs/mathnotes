...
An \textbf{open cover} of a set $E$ in a \@{metric-space} $X$ is a \@{collection} $\{G_\alpha\}$ of \@{open} subsets of $X$ such that $E \subset \bigcup_\alpha G_\alpha.$
\end{definition}
...
\end{theorem}
\begin{proof}
Suppose $E$ is a \@{bounded} infinite subset of $R^k.$ Then, it is a subset of a $k$-cell $I \subset R^k,$ and because \@[every $k$-cell is compact]{every-k-cell-is-compact}, $I$ is compact. Since \@[infinite subsets of a compact set $K$ have a limit point in $K$]{infinite-subset-of-compact-set-has-limit-point}, $E$ has a limit point in $I$ and therefore in $R^k.$
\end{proof}
\begin{note}
This theorem shows up in other forms, especially related to sequences. For example, in my intro real analysis class, it was expressed as the much weaker "Every \@[bounded sequence]{bounded-sequence} in $R^1$ has a \@{convergent} \@{subsequence}." Other equivalent forms are
