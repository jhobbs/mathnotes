...
A set $A$ is said to be \textbf{finite} if $A \sim \naturals_n$ for some $n.$
\end{definition}
...
A set $A$ is said to be \textbf{countable} if there exists a bijection between $A$ and the set of all positive \@{integers} $\integers_{>0}$, that is, if $A \sim \integers_{>0}.$
\end{definition}
...
Now, define $f : \integers_{>0} \to E$ as $f(k) = {x_{k_n}}$, which is a bijection between the positive integers and $E$, showing that $E$ is \@{countable}.
\end{proof}
...
\begin{note}
This means that countably infinite sets are the smallest infinite sets. Any \@{infinite} subset of one has the same \@[cardinal number]{cardinal-number-equivalent} as the parent set, and the same cardinal number as the set of \@{natural-numbers} - $\aleph_0$ - "aleph null."
\end{note}
...
\begin{proof}
\@[Rational numbers]{rational-numbers} just formed from pairs of \@{integers}: $(a, b) \to a/b, b \neq 0,$ so we use the above theorem with $n = 2.$
\end{proof}
