...
A set $X,$ whose \@{elements} we'll call \@{points}, together with a distance \@{function} $d: X \times X \to \reals$ is called a \textbf{metric space} and the distance function $d$ is called a \textbf{metric,} if the following conditions, called the metric axioms, hold for $p, q, r \in X:$
...
$(\reals^n, d : \reals^n \to \reals = | \vec{x} - \vec{y} |)$ is a \@{metric-space} for any $n \geq 0.$
\end{theorem}

\begin{proof}
First, for $n = 0$, $\reals^0$ is just the empty set, so the metric axioms are vacuously satisfied for all points in the set. Now, for $n \geq 1,$
...
Given $\vec{a}, \vec{b} \in \reals^k,$ if $\vec{a}_i < \vec{b}_i$ for all $i = 1, 2, \dots, k,$ then the set of all points $\vec{x}$ who satisfy $\vec{a}_i \leq \vec{x}_i \leq \vec{b}_i,$ $i = 1, 2, \dots, k,$ is called a \textbf{k-cell}. So, a 1-cell is an \@{interval}, a 2-cell is a rectangle, and so on.
\end{definition}

\begin{definition}[Ball]
Given $\vec{x} \in \reals^k, r > 0,$ the open or closed \textbf{ball} with center $\vec{x}$ and radius $r$ is defined as the set of points $\vec{y}$ such that $|\vec{x} - \vec{y}| < r$ or $|\vec{x} - \vec{y}| \leq r,$ respectively.
\end{definition}

\subsection{Convex Sets}

\begin{definition}[Convex Set]
A set $E \subset \reals^k$ is said to be \textbf{convex} if
...
All \@{balls} are \@[convex]{convex-set}.
\end{theorem}
...
So, $\vec{p}$ is within our ball and therefore all balls are \@[convex]{convex-set}.
\end{proof}

\begin{note}
Similar proofs can be used to show that closed \@{balls} and $k$-cells are also \@[convex]{convex-set}.
\end{note}

\begin{theorem}\label{product-of-convex-sets-is-convex}
Let $E \subset \reals^k$ and $F \subset \reals^m$ be \@[convex]{convex-set} sets. Then $E \times F \subset \reals^{k+m}$ is \@[convex]{convex-set}.
\end{theorem}
...
\begin{definition}[Complement (of a Set)]\synonyms{"complement"}
...
$E$ is \textbf{perfect} if $E$ is \@{closed} and if every point of $E$ is a \@{limit-point} of $E.$
\end{definition}
...
A set $E$ is \@{open} iff its \@[complement]{complement-of-a-set} is \@{closed}.
\end{theorem}
...
and by (a) above, (e) is open. Then its \@[complement]{complement-of-a-set}, $\bigcap{\alpha} F_\alpha$ is \@{closed}, and we've shown (b).
...
\begin{proof}
Consider $P = \{\frac{1}{n} | n \in \naturals\}.$ Then $P$ has one \@{limit-point}, $0,$ but $P' = \{0\}$ has no limit points, since the only number that contains $0$ in all its \@{neighborhoods} is $0$ itself (see \@{only-infinite-sets-have-limit-points}.)
\end{proof}
