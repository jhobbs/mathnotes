...
\begin{note}\label{metric-spaces-reference-note}
...
A set $X,$ whose elements we'll call points, together with a distance function $d: X \times X \to \mathbb{R}$ is called a \textbf{metric space} and the distance function $d$ is called a \textbf{metric,} if the following conditions, called the metric axioms, hold for $p, q, r \in X:$
...
\begin{itemize}
\item $d(p,p) = 0.$ (distance is always zero between a point and itself.)
\end{itemize}

\begin{itemize}
\item $d(p,q) = d(q,p).$ (the distance from $p$ to $q$ is the same as the distance from $q$ to $p$.)
\end{itemize}

\begin{itemize}
\item $d(p,q) \leq d(p,r) + d(r,p)$ (triangle inequality.)
\end{itemize}
...
\begin{theorem}\label{euclidean-space-is-metric-space}
...
\begin{itemize}
\item Let $p, q \in R^n, p \neq q.$ Then $|p - q| < 0,$ so $d(p,q) > 0.$
\end{itemize}

\begin{itemize}
\item Let $p \in R^n.$ Now, $p - p = 0,$ so $|p - p| = 0,$ so $d(p,p) = 0.$
\end{itemize}

\begin{itemize}
\item Let $p, q \in R^n.$ $|p - q| = |q - p|,$ so $d(p,q) = d(q, p).$
\end{itemize}

\begin{itemize}
\item Let $p, q, r \in R^n.$ $|p - q| \leq |p - r| + |r - p|,$ so $d(p,q) \leq d(p,r) + d(r, p).$
\end{itemize}
...
\begin{remark}\label{rn-vector-space-vs-metric-space}
...
We could define other, non-norm metrics on $R^n,$ such as the \textbf{discrete metric,} which works for any set $X.$
...
\begin{remark}\label{discrete-metric-satisfies-axioms}
...
\subsection{Segments, Intervals, Cells and Balls}

\begin{definition}[Segment]
A \textbf{segment} $(a, b)$ is the set of all real numbers $x$ such that $a < x < b.$
\end{definition}

\begin{definition}[Interval]
A \textbf{interval} $[a, b]$ is the set of all real numbers $x$ such that $a \leq x \leq b.$
\end{definition}

\begin{definition}[Half-open Interval]
A \textbf{half-open interval} $(a,b]$ or $[a, b)$ is the set of all real numbers such that $a < x \leq b$ or $a \leq x < b,$ respectively.
\end{definition}

\begin{definition}[k-cell]
Given $\vec{a}, \vec{b} \in \mathbb{R}^k,$ if $\vec{a}_i < \vec{b}_i$ for all $i = 1, 2, \dots, k,$ then the set of all points $\vec{x}$ who satisfy $\vec{a}_i \leq \vec{x}_i \leq \vec{b}_i,$ $i = 1, 2, \dots, k,$ is called a \textbf{k-cell}. So, a 1-cell is an interval, a 2-cell is a rectangle, and so on.
\end{definition}

\begin{definition}[Ball]
Given $\vec{x} \in \mathbb{R}^k, r > 0,$ the open or closed \textbf{ball} with center $\vec{x}$ and radius $r$ is defined as the set of points $\vec{y}$ such that $|\vec{x} - \vec{y}| < r$ or $|\vec{x} - \vec{y}| \leq r,$ respectively.
\end{definition}

\subsection{Convex Sets}

\begin{definition}[Convex Set]
A set $E \subset \mathbb{R}^k$ is said to be \textbf{convex} if
...
\begin{theorem}\label{balls-are-convex}
...
\subsection{Elements and Subsets of a Metric Space}

\begin{note}\label{metric-space-context}
Assume we have a metric space $(X, d).$
\end{note}

\begin{definition}[Neighborhood]
A \textbf{neighborhood,} or \textbf{r-neighborhood} of $p$ is a set $N_r(p)$ consisting of all $q$ such that $d(p, q) < r$ for some $r > 0.$ This subset of $X$ is all the points within a circle of radius $r$ - the open ball of radius $r$ centered at $p.$
\end{definition}

\begin{definition}[Limit Point]
A point $p$ is a \textbf{limit point} of the set $E$ if every neighborhood of $p$ contains a point $q \neq p$ such that $q \in E.$
\end{definition}

\begin{definition}[Isolated Point]
If $p \in E$ and $p$ is not a limit point of $E,$ then $p$ is called an \textbf{isolated point} of E.
\end{definition}

\begin{definition}[Closed]
A set $E$ is \textbf{closed} if every limit point of $E$ is a point of $E.$
\end{definition}

\begin{definition}[Interior Point]
A point $p$ is an \textbf{interior point} of $E$ if there is a neighborhood $N$ of $p$ such that $N \subset E.$
\end{definition}

\begin{definition}[Open]
A set $E$ is open if every \dref{point} of $E$ is an \dref{interior-point} of $E.$
\end{definition}

\begin{definition}[Complement (of a Set)]
The \textbf{complement} of $E$ (denoted by $E^c$) is the set of all points $p \in X$ such that $p \notin E.$
\end{definition}

\begin{definition}[Perfect Set]\synonyms{perfect}
$E$ is \textbf{perfect} if $E$ is \dref{closed} and if every point of $E$ is a \dref{limit-point} of $E.$
\end{definition}

\begin{definition}[Bounded]
$E$ is \textbf{bounded} if there is a real number $M$ and a point $q \in X$ such that $d(p, q) < M$ for all $p \in E.$
\end{definition}

\begin{definition}[Dense]
$E$ is \textbf{dense} in $X$ if every point of $X$ is a limit point of $E,$ or a point of $E$ (or both.)
\end{definition}

\begin{theorem}\label{every-neighborhood-is-an-open-set}
...
\includedemo{neighborhood}

\begin{theorem}\label{neighborhood-of-limit-point-contains-infinitely-many-points}
...
\begin{corollary}\label{only-infinite-sets-have-limit-points}
...
\begin{theorem}\label{complement-of-union-is-intersection-of-complements-general}
...
\begin{note}
This is just \dref[De Morgan's law]{complement-of-union-is-intersection-of-complements} extended to arbitrary indexed collections.
\end{note}

\begin{theorem}\label{open-iff-complement-closed}
...
\begin{remark}\label{open-closed-intuition}
...
\begin{theorem}\label{union-and-intersection-of-open-and-closed-sets}
...
\begin{theorem}\label{limit-points-form-closed-set}
...
If $X$ is a metric space, $E \subset X,$ and $E'$ denotes the set of all limit points of $E$ in $X,$ then the \textbf{closure} of $E$ is the set $\closure{E} = E \cup E'.$
\end{definition}

\begin{theorem}\label{set-is-its-closure-iff-it-is-closed}
...
\begin{theorem}\label{set-and-closure-have-same-limit-points}
If $E$ is a set in a metric space, then $E$ and $\closure{E}$ have the same limit points.
\end{theorem}

\begin{proof}
If $E$ is closed, then we are done, because \dref[a set equals its closure if it is closed]{set-is-its-closure-iff-it-is-closed}. 
...
Conversely, suppose $p$ is a limit point of $\closure{E}.$ Then, every neighborhood $N$ of $p$ contains a point of $q \in \closure{E}, q \neq p.$ If $q \in E,$ then $N$ clearly contains a point in $E.$ Otherwise, $q \in E', q \not in E.$ Now, since $q \in N$ and \dref[every neighborhood is an open set]{every-neighborhood-is-an-open-set}, $q$ has some neighborhood $M \subset N.$ Since $q \in E',$ $M$ contains some point $s \in E.$ Since $M \subset N,$ $s \in N,$ and therefore $N$ contains a point in $E.$ Thus, all neighborhoods of $p$ contain some point in $E,$ and $p$ is a limit point of $E.$
\end{proof}

\begin{theorem}\label{set-and-its-limit-points-may-have-different-limit-points}
A set $E$ and its limit points $E'$ do not necessarily have the same limit points.
\end{theorem}

\begin{proof}
Consider $P = \{\frac{1}{n} | n \in \mathbb{N}\}.$ Then $P$ has one limit point, $0,$ but $P' = \{0\}$ has no limit points, since the only number that contains $0$ in all its neighborhoods is $0$ itself (see \dref{only-infinite-sets-have-limit-points}.)
\end{proof}

\begin{theorem}\label{closure-distributes-over-finite-unions}
...
Suppose $E \subset Y \subset X,$ and $X$ is a metric space. We say that $E$ is \textbf{open relative} to $Y$ if to each $p \in E$ there is associated an $r > 0$ such that $q \in E, q \in Y$ whenever $d(p, q) < r.$
\end{definition}

\begin{theorem}\label{open-relative-iff-intersection-with-open-subset}
