...
The system $\mathcal{F}$ is said to be \textbf{recursive} (or \textbf{effectively generated}) if $W$, $A$, and $R$ are all \@{recursively-enumerable} sets, so that \@{derivations} can be verified by a finite mechanical procedure.
\end{definition}
