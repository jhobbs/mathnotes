...
\textbf{Angular frequency} (usually denoted as $\omega$) is the number of radians completed per unit of time. There are $2 \pi$ radians per cycle of a periodic function, so angular frequency is related to frequency by:
...
\href{https://www.desmos.com/calculator/jzxw6vcm9d}{Here is a desmos graph of parameterized sin}

To make a $\sin$ function with frequency $f$, use $\sin{(2 \pi f t)}$. Some examples:

\begin{itemize}
\item If we want a $\sin$ wave with a frequency of $1$ hz, we use $\sin{(2 \pi t)}$
\item If we want a $\sin$ wave with a frequency of $50$ hz, we use $\sin{(100 \pi t)}$
\item If we want a $\sin$ wave with an angular frequency of $50$ radians per second, we use $\sin{50t}$
\item If we want a $\sin$ wave with a period of $50$, we use $\sin{\left ( 2 \pi \frac{1}{50} t \right )}$
\end{itemize}
