...
In a \dref{dynamical-system} with a smoothly varying \dref{parameter} $r,$ a sudden change in qualitative or topological behavior of the system, especially the creation, annihilation, or change of stability in its \dref{fixed-points}, is called a \textbf{bifurcation.}
\end{definition}

\begin{definition}[bifurcation point]
The parameter values at which \dref{bifurcations} occur are called \textbf{bifurcation points.}
\end{definition}

You can see some \dref{bifurcations} in action with the demo below.

\includedemo{parametric-phase-portrait}

\subsection{Saddle-Node Bifurcation}

\begin{definition}[Saddle-Node]
The defining feature of a \textbf{saddle-node} \dref{bifurcation} is that as $r$ crosses the \dref{bifurcation-point}, two \dref{fixed-points} are created or destroyed (depending on the direction of approach.) If approaching from the direction in which the \dref{fixed-points} exist, they will grow closer to each other, until they reach other at the \dref{bifurcation-point}, forming a \dref{half-stable} point, and then they will be destroyed as $r$ continues on the other side of the \dref{bifurcation-point}.
\end{definition}

\begin{definition}[Tangential Intersection]
Two \dref{functions} $f$ and $g$ are said to have a \textbf{tangential intersection} at a \dref{point} $p$ if both their \dref{values} and the \dref{values} of their first \dref{derivatives} are equal at $p.$ That is, if

\[ f(p) = g(p) \quad \text{AND} \quad f'(p) = g'(p). \]
\end{definition}

I don't have a full definition for \textbf{normal form} yet, but the prototypical forms for \dref{saddle-node} \dref{bifurcations} are

\[ \dot{x} = r - x^2, \quad \dot{x} = r + x^2. \]

For any $f(x, r),$ if we have the following conditions at the \dref{bifurcation} point $r^*,$ the \dref{dynamical-system} can be reduced to one of the \dref{saddle-node} normal forms above:

\[ f(x^*, r^*) = 0, \quad \partial_x f(x^*, r^*) = 0, \]

that is, $f(x^*, r^*)$ is a \dref{tangential-intersection} with the $x$-axis, and

\[ \partial_{xx} f(x^*, r^*) \neq 0, \partial_r f(x^*, r^*) \neq 0. \]

The first ensures this is only a double root, not a triple root or worse. The second ensures that as we vary $r$ near this point, the function value actually changes, which is required to create or destroy the \dref{fixed-points}.

\subsection{Transcritical Bifurcation}

\begin{definition}[Transcritical]
A \textbf{transcritical} bifurcation occurs when a given $x^*$ is always a \dref{fixed-point}, but its stability changes when crossing the \dref{bifurcation-point} $r^*$. At all values of $r$ other than $r^*,$ there are two \dref{fixed-points} (and at $r^*$ one degenerate \dref{fixed-point}), but their stability is exchanged at the crossing.
\end{definition}

The normal form for a \dref{transcritical-bifurcation} is $\dot{x} = r x - x^2.$
...
\begin{intuition}
The \dref{fixed-point} doesn't persist as $r$ varies $(f_r \neq 0)$ and the nullcline has quadratic tangency $(f_{xx} \neq 0)$, so two \dref{fixed-points} collide and annihilate.
\end{intuition}
...
\begin{intuition}
The \dref{fixed-point} exists for all $r  ~ (f_r = 0),$ but two branches cross and exchange stability. The $f_{xr} \neq 0$ ensures the stability actually changes (the \dref{eigenvalue} $f_x$ crosses zero with nonzero speed as $r$ varies).
\end{intuition}
...
The \dref{fixed-point} exists for all $r ~ (f_r = 0),$ and there's odd symmetry in $x ~ (f_{xx} = 0),$ so instead of two branches crossing, one branch splits into three. The cubic term dominates.
...
\subsection{Algorithm for analyzing bifurcations and drawing a diagram}

\textbf{Algorithm (1D bifurcation diagram for $\dot{x} = f(x,r)$)}
...
\begin{itemize}
\item If the branch is given as $x = x_i(r),$ compute
\end{itemize}

\[ f_x\bigl(x_i(r),\, r\bigr). \]

\begin{itemize}
\item If the branch is given as $r = r_i(x),$ compute
\end{itemize}
