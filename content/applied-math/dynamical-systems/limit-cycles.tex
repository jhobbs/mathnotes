...
Given a system $\dot{\vec{x}} = \vec{f(x)},$ a \textbf{conserved quantity} is a \@{real-valued} \@{continuous} \@{function} $E(\vec{x})$ that is constant on \@{trajectories}, i.e. $dE/dt = 0.$ We also require that $E(\vec{x})$ is nonconstant on every \@{open} set, to avoid trivial examples such as $E(\vec{x}) = 0.$
\end{definition}
...
A \@{trajectory} that starts and ends at the same \@{fixed-point} is called a \textbf{homoclinic orbit.}
\end{definition}
...
Pairs of orbits connecting twin \@{saddle-points} are called \textbf{heteroclinic trajectories.}
\end{definition}
...
A \textbf{limit cycle} is an isolated \@{closed} \@{trajectory}. Isolated means that \@{neighboring} \@{trajectories} are not \@{closed}; they spiral toward or away from the limit cycle.
\end{definition}

\begin{definition}[Stable limit cycle]\synonyms{"attracting limit cycle"}
If all neighboring \@{trajectories} approach the \@{limit-cycle}, we say the \@{limit-cycle} is \textbf{stable} or \textbf{attracting.}
\end{definition}

\begin{definition}[Unstable limit cycle]
If a \@{limit-cycle} is not stable, we say it is an \textbf{unstable limit cycle}, or in exceptional cases, half-stable.
\end{definition}
...
If a \@{system} can be written in the form $\dot{\vec{x}} = - \nabla V,$ for some \@{continuously-differentiable}, single-valued \@{scalar-function} $V(\vec{x}),$ then it is said to be a \textbf{gradient system} with \textbf{potential function} $V.$
\end{definition}
...
\begin{intuition}
If we're always moving "downhill" in some direction in a space, it's impossible to come back to where we started. This is another reason why oscillations aren't possible in \@[one dimensional systems]{one-dimensional-systems}.
\end{intuition}
