...
\title{Discrete Time Dynamical Systems}

\begin{document}

\section{Discrete Time Dynamical Systems}

We'll go over systems in which time is discrete rather than continuous. These systems are called \dref{difference-equations}, \dref{recursion-relations}, iterated maps, or simply maps. These can be interesting; even in one dimension they can exhibit oscillations and chaos.
...
Solutions to \dref{difference-equations} are \dref{sequences} $(x_0, x_1, ..., x_n).$ If $x_0$ is the initial condition, then
...
So the solution is $(x_1, x_2, \dots, x_n).$ Long term dynamics are given by moving along this \dref{sequence} as $n \to \infty.$

We will consider ourselves, for now, with \dref{autonomous-systems} where there is no explicit dependence on $n,$ i.e.
...
\begin{definition}[orbit]\synonyms{solution}
A \dref{sequence} $(x_0, x_1, x_2, \dots)$ is called the \textbf{orbit} (or solution) starting from $x_0.$ This is analogous to a \dref{trajectory} in a continuous time dynamical system.
\end{definition}
...
\subsection{Stability of Linear Maps}
...
% TODO: Linear stability analysis for nonlinear maps: linearize about x\emph{, criterion |f'(x})| < 1 stable, > 1 unstable

% TODO: Higher-dimensional discrete systems x\_\{n+1\} = A x\_n, Jacobian criterion: all eigenvalues inside the unit circle
