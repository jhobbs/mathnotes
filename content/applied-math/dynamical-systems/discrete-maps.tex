...
where $n$ is a time-like variable, $\vec{f}$ and $\vec{x_n}$ may be \@{vectors}, $n = 0, 1, 2, \dots = \naturals_0.$
\end{definition}
...
So $x_n \in \reals^N$ and $f : \reals^N \to \reals^N. $
...
A \@{sequence} $(x_0, x_1, x_2, \dots)$ is called the \textbf{orbit} (or solution) starting from $x_0.$ This is analogous to a \@{trajectory} in a continuous time dynamical system.
\end{definition}
...
\[ x_{n+1} = \lambda x_n, \quad \lambda \in \reals. \]

Starting with $x_0,$ $x_1 = \lambda x_0, x_2 = \lambda x_1 = \lambda ( \lambda x_0) = \lambda^2 x_0, \dots,$ so our general solution is

\[ x_n = \lambda^n x_0. \]

\@{Fixed-points} satisfy $x^* = f(x^*) \implies x^* = \lambda x^* \implies x^*(\lambda - 1) = 0.$ Therefore, $x^* = 0$ is always a fixed point, and if $lambda = 1,$ then every $x^* \in \reals$ is a fixed point, i.e. the map is just $x_{n+1} = x_n.$ 
