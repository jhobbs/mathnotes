...
\title{Weakly Nonlinear Oscillators}

\begin{document}

\section{Perturbation Theory}

\begin{definition}[Weakly Nonlinear Oscillator]
Equations of the form

\[ \dot{x} + x + \epsilon h(x, \dot{x}) = 0 \tag{a} \]

where $0 \leq \epsilon \ll 1$ and $h(x, \dot{x})$ is an arbitrary \@{smooth} \@{function} represent small \@{perturbations} of the \@{linear} \@{oscillator} $\dot{x} + x = 0$ and are therefore called \textbf{weakly nonlinear oscillators.}
\end{definition}
...
where we have to determine $x_k(t)$ from the original equation and initial conditions. Ideally, we could get a useful solution just truncating to the first few terms and the higher order terms would just be minor corrections. This approach is called \textbf{regular perturbation theory.}
...
The general solution for the $O(1)$ equation, whose eigenvalues are $\pm i,$ is $x_0 = c_1(T) e^{i\tau} + c_2(T) e^{-i \tau}, c_1, c_2 \in \complexes.$ But we have $x$ is real, so $c_1(T) = \overline{c_2(T)},$ and we can write our solution as
