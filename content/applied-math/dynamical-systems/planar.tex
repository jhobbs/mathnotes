\documentclass{article}
\usepackage{mathnotes}

\title{Planar Systems}

\begin{document}

Here we'll talk about both linear and nonlinear planar systems of the form

\[ \begin{aligned}
\dot{x} & = f(x,y) \\
\dot{y} & = g(x,y).
\end{aligned} \]

\begin{definition}[Separatrix]
A \textbf{separatrix} is the boundary that separating two modes of behavior in a dynamical system.
\end{definition}

\begin{definition}[Homoclinic Orbit]
Trajectories that start and end at the same \dref{fixed-point} are called \textbf{homoclinic orbits.}
\end{definition}

\begin{definition}[Heteroclinc Trajectory]
Trajectories that connect two \dref{fixed-point} are called \textbf{heteroclinic orbits.}
\end{definition}

\begin{definition}[Hyperbolic Fixed Point]
A \textbf{hyperbolic fixed point} is a \dref{fixed-point} for which the real part of both eigenvalues is non-zero.
\end{definition}

\subsection{Nullclines}

\begin{definition}[Nullcline]
The nullclines of a planar system are the curves where either $\dot{x} = 0$ or $\dot{y} = 0$ and indicate where the flow is either purely horizontal or purely vertical. \dref{fixed-points} occur at intersections of nullclines.
\end{definition}

\subsection{Classification of Linear Systems}

We can write a linear planar system as

\[ \dot{\vec{x}} = A \vec{x}, \quad \vec{x} \in \mathbb{R}^2 \]

where

\[ A = \begin{pmatrix} a & b \\ c & d \end{pmatrix}. \]

\begin{definition}[Characteristic equation]
The characteristic equation tells us how to find the \dref{eigenvalues} and is given by:

\[ \lambda^2 - \tau \lambda + \Delta = 0. \]
\end{definition}

Rewriting that using the quadratic equation gives:

\[ \lambda_{1,2} = \frac{1}{2} \left (\tau \pm \sqrt{\tau^2 - 4 \Delta} \right ) \]

where

\[ \tau = \tr{(A)} = a + d = \lambda_1 + \lambda_2, \quad \Delta = \det{(A)} = ad - bc = \lambda_1 \lambda_2. \]

\begin{tabular}{ll}
Type & Condition \\
\hline
Stable Node & $\lambda_1,\lambda_2 < 0,$ real \\
Unstable Node & $\lambda_1, \lambda_2 > 0,$ real \\
Saddle & $\Delta < 0$, equiv $\lambda_1 < 0 < \lambda_2$ \\
Stable Spiral & $\Re{\lambda} < 0,$ complex \\
Unstable Spiral & $\Re{\lambda} > 0,$ complex \\
Center & $\Re{\lambda} = 0,$ complex (purely imaginary) \\
\end{tabular}

Linear systems only have one fixed point, the origin!

\subsection{Jacobian Linearization}

Given general (can be nonlinear planar system:

\[ \dot{x} = f(x,y), \quad \dot{y} = g(x,y), \]

if we have a fixed point $(x^*, y^*),$

then

\[ f(x^*, y^*) = 0, \quad g(x^*, y^*) = 0. \]

We can do linear stability analysis on this system to understand the local behavior near fixed points. We use the \dref{Jacobian} matrix of the system, then plug in $(x^*, y^*).$ For the nonlinear case, we have the same geometric interpretations as the linear case. Topologically, if the real part of any eigenvalue of a fixed point is positive, then the fixed point is unstable. If the real part of all eigenvalues of a fixed point is negative, then the fixed point is asymptotically stable. We call these cases (both eigenvalues have nonzero real parts) hyperbolic \dref{fixed} points

However, if the real part of any eigenvalue is zero, this linearized approached does not tell us about the stability of the fixed point.

\end{document}
