...
Trajectories that start and end at the same \@{fixed-point} are called \textbf{homoclinic orbits.}
\end{definition}

\begin{definition}[Heteroclinc Trajectory]
Trajectories that connect two \@{fixed-point} are called \textbf{heteroclinic orbits.}
\end{definition}

\begin{definition}[Hyperbolic Fixed Point]
A \textbf{hyperbolic fixed point} is a \@{fixed-point} for which the real part of both eigenvalues is non-zero.
\end{definition}
...
\[ \dot{\vec{x}} = A \vec{x}, \quad \vec{x} \in \reals^2 \]
