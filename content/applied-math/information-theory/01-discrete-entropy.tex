...
A system whereby a \dref{sequence} of choices from a \dref{finite} \dref{set} of elementary symbols $S_1, \dots, S_n$ can be transmitted from one point to another. Each of the symbols $S_i$ is assumed to have a certain duration in time $t_i$ seconds (not necessarily the same for different $S_i$).
\end{definition}

\begin{definition}[Capacity]\synonyms{"channel-capacity"}
The \textbf{capacity} $C$ of a \dref{discrete-channel} is given by

\[ C = \lim_{t \to \infty} \frac{\log{N(t)}}{t}, \]

where $N(t)$ is the number of allowed \dref{signals} of duration $t.$
\end{definition}
...
If $N(t)$ represents the number of \dref{sequences} of duration $t,$ then
...
where $A$ is constant and $W_0$ is the largest real solution of the \dref{characteristic-equation}:
...
We can think of these states being the \dref{nodes} of a \dref{directed-graph} with the transitions between them being the \dref{edges}. A generalization of this is given in the theorem below.
\end{example}

\begin{theorem}
Let $b_{ij}^{(s)}$ be the duration of the $s$-th symbol which is allowable in state $i$ and leads to state $j.$ Then the \dref{channel-capacity} $C$ is equal to $\log{W}$ where $W$ is the largest real root of the \dref{determinantal-equation}

\[ \left | \sum_{s} W^{-b_{ij}^{(s)}} - \delta_{ij}  \right | = 0, \]

where $\delta_{ij}$ is the \dref{Kronecker-delta}.
\end{theorem}
...
Expanding the \dref{determinant} on the LHS gives the \dref{characteristic-equation} $(a)$ above.
\end{example}
...
Given a real number $b > 1$ and an outcome $x$ of a discrete \dref{random-variable} $X$ with probability mass function $p,$ the \textbf{self-information} of $x$ is defined as the negative \dref{log-probability}

\[ I(x) := - \log_{b}{p(x)}. \]
\end{definition}

\begin{remark}
We can view \dref{self-information} as an alternative casting of \dref{probability}, like how \dref{odds} are, with some desirable properties:
...
\begin{itemize}
\item The less probable an event is, the more \dref{surprisal} it yields.
\end{itemize}

\begin{itemize}
\item \dref{self-information} is \dref{additive} - if two individual events are measured separately, the total amount of information is the sum of the \dref{self-informations} of the individual events.
\end{itemize}
...
\begin{enumerate}
\item $H$ should be \dref{continuous} in the $p_i.$ That is, a small change in a $p_i$ should result in a small change in $H.$
\end{enumerate}

\begin{enumerate}
\item If all the $p_i$ are equal, $p_i = \frac{1}{n},$ then $H$ should be a \dref{monotonic} \dref{increasing} \dref{function} of $n.$ With equally likely events there is more choice, or uncertainty, when there are more possible events.
\end{enumerate}
...
Given a discrete \dref{random-variable} $X,$ which may be any \dref{element} $x$ within the set $\mathcal{X},$ and is distributed according to $p: \mathcal{X} \to [0,1],$ the \textbf{entropy} is

\[ H(X) := - \sum_{x \in \mathcal{X}} p(x) \log{p(x)}. \]

We can choose different bases for the logarithm; throughout these notes a bare $\log$ means $\log_2,$ giving the unit of \dref{bits}.

An alternative, equivalent definition is that \textbf{entropy} is the \dref{expected-value} of the \dref{self-information} of a \dref{random-variable}:
...
Real numbers $a_1, \dots, a_n$ are \textbf{commensurable} if there exists a common measure $m \in \mathbb{R}_{>0}$ such that each $a_i$ is an integer multiple of it:
...
\begin{theorem}
The only $H$ satisfying the three required properties above is the \dref{entropy} function defined above, up to multiplication by a constant.
\end{theorem}
...
By property (2) of $A(n)$ (it is a \dref{monotonically-increasing} function of $n$,)  
...
Now, by the \dref{triangle-inequality}, we have that
...
If the $p_i$ are incommensurable, we can approximate them as closely as we'd like with \dref{rationals}, since \dref{rationals-are-dense-in-reals}. By the first property we assumed for $H,$ it is \dref{continuous} in the $p_i,$ and so its value at the incommensurable $p_i$ equals its \dref{limit} as we approach via the \dref{rationals}, and so our expression holds in general. $K$ is left to us to pick, picking it is equivalent to picking a base for the logarithm.
\end{proof}

\detach

\begin{remark}
The \dref{entropy} of a \dref{random-variable} quantifies the average level of uncertainty or information associated with the variable's possible outcomes. It measures the expected amount of information needed to describe the state of the variable, considering the distribution of probabilities across all potential states.
\end{remark}
...
where the skew $\alpha$ controls how unevenly probability mass is spread across symbols. At $\alpha = 0$ every symbol is equally likely (the uniform distribution), so \dref{entropy} is maximal at $\log N.$ As $\alpha$ grows the distribution concentrates on the first few symbols, the rare symbols carry ever more \dref{self-information}, and the \dref{entropy} falls toward $0.$ The bar chart shows the probability of each symbol alongside its \dref{self-information}; the second chart traces the \dref{entropy} as $\alpha$ sweeps from uniform to highly skewed.
...
\begin{theorem}
When a \dref{random-variable} is uniformly distributed over an alphabet of $n$ \dref{elements}, the \dref{self-information} of any given \dref{element} equals the \dref{entropy} of the \dref{random-variable} and is $\log{n}.$
\end{theorem}
...
\begin{theorem}
A \dref{random-variable} has the most \dref{entropy} when the \dref{elements} of its alphabet are all equally likely to occur.
\end{theorem}

\begin{proof}
We will use a \dref{Lagrangian} function of \dref{entropy} to show this. Let
...
It remains to show that this extreme of $H$ is a maximum. This follows from the facts that the domain is a \dref{convex}, \dref{compact} set and that the entropy function is strictly \dref{concave}. TODO: more details on these.
\end{proof}

\begin{theorem}\label{joint-entropy-is-less-than-or-equal-to-entropy-of-parts}
Let $X$ and $Y$ be discrete \dref{random-variables} with alphabets $\mathcal{X}$ and $\mathcal{Y},$ and let $p(x,y)$ be the probability of the joint occurrence of $X = x$ and $Y = y.$

The \dref{entropy} of the joint event, $H(X,Y),$ is less than or equal to the sum of the individual entropies, i.e.
...
\begin{proof}
The \dref{entropy} of the joint event is
...
Now, by \dref{jensens-inequality}, because \dref{log-is-concave}, we have that $\mathbb{E}[\log{Z}] \leq \log{\mathbb{E}[Z]}.$ Now,
...
Now, for the equality part. Suppose $H(X) + H(Y) = H(X,Y).$ Then, $\mathbb{E}[\log{Z}] = 0,$ and recalling that $\mathbb{E}[Z] = 1,$ $\mathbb{E}[\log{Z}] = \log{\mathbb{E}[Z]} = 0.$ In \dref{jensens-inequality}, equality holds only if our function is \dref{affine} or if $Z$ is constant; since $\log$ is \dref{affine} on no interval, $Z$ must be constant, and since $\mathbb{E}[Z] = 1,$ $Z = \frac{p(x)p(y)}{p(x,y)} = 1 \implies p(x)p(y) = p(x,y).$ 
...
\begin{note}
When we decrease the probability of an \dref{element} occurring, its \dref{self-information} increases only logarithmically, while its weight in the entropy sum decreases linearly.
\end{note}

\begin{definition}[Cross-Entropy]
Given two discrete probability distributions, $p$ and $q,$ that share a support $\mathcal{X},$ \textbf{cross-entropy} measures the average number of bits needed to represent an event drawn from $\mathcal{X}$ when the coding scheme is optimized for an estimated distribution $q$ rather than the true distribution $p.$ Its formula is very similar to that of \dref{entropy}, except \dref{surprisal} is calculated using $q$ while \dref{expectation} uses $p$:

\[ H(p,q) = - \sum_{x \in \mathcal{X}} p(x) \log{q(x)}. \]

We can write it in \dref{expectation} form as
...
Note that when $p = q,$ \dref{cross-entropy} equals \dref{entropy}. The symbol $H$ is overloaded: $H(p,q)$ with distributions as arguments means \dref{cross-entropy}, while $H(X,Y)$ with \dref{random-variables} as arguments means joint entropy.
...
which is just \dref{cross-entropy} minus \dref{entropy}:
...
\begin{remark}
\dref{kl-divergence} tells us how different an approximating distribution $q$ is from a true probability distribution $p.$ It's not a true distance metric - it's asymmetric, but it does behave like distance in that it gets smaller as $q$ gets more like $p$ and is zero when $q = p.$ It is a sort of directed distance.
...
with equality iff $p = q.$ Equivalently, \dref{cross-entropy} is never less than \dref{entropy}: $H(p,q) \geq H(p),$ with equality iff $q = p.$
\end{theorem}

\begin{proof}
Because \dref{log-is-concave}, by \dref{jensens-inequality} we have that
...
Now, note that if $p = q,$ $\frac{q_i}{p_i} = 1$ and $D(p || q) = 0.$ Conversely, suppose $D(p || q) = 0.$ Since $\log$ is strictly \dref{concave}, it is not \dref{affine}, and \dref{jensens-inequality} condition for equality gives us that $\frac{q_i}{p_i} = c.$ Now, $\sum_i q_i = \sum_i c p_i = 1,$ and since $p$ is a probability distribution, $c = 1,$ and therefore $p_i = q_i$ and $p = q.$
\end{proof}

\begin{theorem}[Doubly stochastic maps increase entropy]
Take a \dref{doubly-stochastic} \dref{matrix} $A$ and \dref{probability-vector} $p,$ let $q = A p.$ Then $H(q) \geq H(p),$ with equality iff $q$ is a \dref{rearrangement} of $p$.
\end{theorem}
...
Now we find the \dref{relative-entropy} from $s$ to $r$:
...
So, we end up with $D(r || s) = H(q) - H(p).$ Since $D(r || s) \geq 0$ by \dref{gibbs-inequality}, we have $H(q) - H(p) \geq 0,$ which implies that $H(q) \geq H(p),$ which is what we wanted to show.
...
\begin{intuition}
Averaging (well, replacing each probability with a \dref{convex-combination} of all the probabilities in a way that retains their summing to 1) the probabilities in $p$ can only bring them closer to each other (if it doesn't just rearrange them), which increases entropy.
\end{intuition}

\begin{definition}[Conditional Entropy]
Let $X$ and $Y$ be \dref{random-variables}, not necessarily \dref{independent}. The probability that $Y$ takes the value $y$ when $X$ takes the value $x$ (for any specific $x \in \mathcal{X}, y \in \mathcal{Y}$) is called the \dref{conditional-probability} $p(y|x)$ and is given by:

\[ p(y|x) = \frac{p(x, y)}{p(x)}, \quad p(x) = \sum_{y} p(x,y). \]

(Note that $p(x,y)$ is the \dref{joint-probability} of $X = x, Y = y.$)
...
\begin{note}
We can obtain the above formula for \dref{conditional-entropy} as follows. First, if $X = x,$ then our \dref{entropy} for $Y$ is
...
\begin{theorem}\label{joint-entropy-is-base-plus-conditional}
The \dref{entropy} of the joint event $X,Y$ is the \dref{entropy} of $X$ plus the \dref{entropy} of $Y$ when $X$ is known.
\end{theorem}
...
\begin{theorem}
The \dref{uncertainty} of $Y$ is never increased by knowledge of $X.$ It will be decreased unless $X$ and $Y$ are independent events, in which case it is not changed.
\end{theorem}

\begin{proof}
From \dref{joint-entropy-is-less-than-or-equal-to-entropy-of-parts} and \dref{joint-entropy-is-base-plus-conditional}, we have
