...
\begin{lemma}[Nonnegativity of Entropy]
\[ H(X) \geq 0. \]
\begin{proof}
	\[ 0 \leq p(x) \leq 1 \implies \log{p(x)} \leq 0 \implies - \log{p(x)} \geq 0. \]
\end{proof}
\end{lemma}

\begin{lemma}[Change of Logarithm Base]
\[ H_b(X) = (\log_b{a})H_a(X). \]
...
\bal
    H_b(x) & = - \sum_{x \in \mathcal{X}} p(x) \log_b{x} \\
           & = - (\log_b{a}) \sum_{x \in \mathcal{X}} p(x) \frac{\log_b{x}}{\log_b{a}} \\
           & = - (\log_b{a}) \sum_{x \in \mathcal{X}} p(x) \log_a{x} \\
	   & = (\log_b{a})H_a(X)
\eal
\end{proof}
...
\begin{corollary}[Chain Rule for Conditional Joint Entropy]\label{chain-rule-for-conditional-joint-entropy}
...
\begin{proof}
\bal
	H(X, Y|Z) & = \sum_{z \in \mathcal{Z}} p(z) H(X,Y | Z = z) \\
	          & = \sum_{z \in \mathcal{Z}} p(z) \left ( H(X | Z = z ) + H(Y|X,Z=z) \right ) \\
	          & = \sum_{z \in \mathcal{Z}} p(z)  H(X | Z = z ) + \sum_{z \in \mathcal{Z}} p(z) H(Y|X,Z=z) \\
		  & = H(X | Z) + H(Y | X, Z).
\eal
\end{proof}
...
\begin{definition}[KL Divergence]\synonyms{relative-entropy, "Kullback-Leibler divergence"}
	For discrete probability distributions, $p$ and $q,$ on a common \@{alphabet} $\mathcal{X},$ the \textbf{relative entropy} from $q$ to $p$ is defined to be
...
\begin{definition}[Mutual Information]\label{mutual-information}
	Consider two \@{random-variables} $X$ and $Y$ with \@{joint probability mass function} $p(x,y)$ and \@{marginal probability mass functions} $p(x)$ and $p(y).$ The \textbf{mutual information} $I(X; Y)$ is the \@{relative-entropy} between the \@{joint distribution} and the \@{product-distribution} $p(x)p(y)$:
...
\subsection{Relationship Between Entropy and Mutual Information}

\begin{theorem}[Mutual Information and Entropy]\label{mutual-information-and-entropy}

\bal
	I(X; Y) & = H(X) - H(X|Y) & & \text{(a)} \\
	I(X; Y) & = H(Y) - H(Y|X) & & \text{(b)} \\
	I(X; Y) & = H(X) + H(Y) - H(X,Y) & & \text{(c)} \\
	I(X; Y) & = I(Y; X) & & \text{(d)} \\
	I(X; X) & = H(X). & & \text{(e)}
\eal
...
\bal
	I(X; Y) & = \sum_{x \in \mathcal{X}} \sum_{y \in \mathcal{Y}} p(x,y) \log{\frac{p(x,y)}{p(x)p(y)}} \\
	        & = \sum_{x \in \mathcal{X}} \sum_{y \in \mathcal{Y}} p(x,y) ( \log{p(x,y)} - \log{p(x)p(y)} ) \\
		& = -H(X,Y) - \sum_{x \in \mathcal{X}} \sum_{y \in \mathcal{Y}} p(x,y) \log{p(x)p(y)} \\
		& = -H(X,Y) - \sum_{x \in \mathcal{X}} \sum_{y \in \mathcal{Y}} p(x,y) ( \log{p(x)} + \log{p(y)} ) \\
		& = -H(X,Y) - \sum_{x \in \mathcal{X}} \sum_{y \in \mathcal{Y}} p(x,y) \log{p(x)} - \sum_{x \in \mathcal{X}} \sum_{y \in \mathcal{Y}} p(x,y) \log{p(y)} \\
		& = H(X) + H(Y) - H(X,Y)
\eal


	Now, from \@{joint-entropy-is-base-plus-conditional} we prove (a) and (b) with

\bal
	H(X) + H(Y) - H(X,Y) & = H(X) + H(Y) - H(Y) - H(X|Y) \\
	                     & = H(X) - H(X|Y).
\eal
...
\eal
	...
	(d) is trivial from symmetry of $p(a,b) = p(b,a)$ and $a * b = b * a.$
...
\begin{theorem}[Chain Rule for Entropy]\label{chain-rule-for-entropy}
...
\end{theorem}
\begin{proof}
	From \@{joint-entropy-is-base-plus-conditional}, we have
...
\bal
	...
	H(X_1, X_2, X_3) & = H(X_1) + H(X_2, X_3 | X_1) \\
	                 ...
	                 & \ \vdots \\
	H(X_1, X_2, \dots, X_n) & = H(X_1) + H(X_2|X_1) + \cdots + H(X_n | X_{n-1}, \dots, X_1) \\
...
\bal
	H(X_1, X_2, \dots, X_n) - H(X_1, X_2, \dots, X_n | Y ) & = \sum_{i=1}^n H(X_i | X_{i-1}, \dots, X_1) - \sum_{i=1}^n H(X_i | X_{i-1}, \dots, X_1, Y) \\
	& = \sum_{i=1}^n H(X_i | X_{i-1}, \dots, X_1) -  H(X_i | Y, X_{i-1}, \dots, X_1).
\eal

	Now, from our definition of \@{conditional-mutual-information} we get

	\[ \sum_{i=1}^n H(X_i | X_{i-1}, \dots, X_1) -  H(X_i | Y, X_{i-1}, \dots, X_1) = \sum_{i=1}^n I(X_i; Y|X_1, X_2, \dots, X_{i-1}). \] 
\end{proof}

\begin{definition}[Conditional Relative Entropy]
	For \@{joint probability mass functions} $p(x,y)$ and $q(x,y),$ the \textbf{conditional relative entropy}
...
\end{theorem}
\begin{proof}
	Using \@{relative-entropy}, \@{joint-is-marginal-times-conditional}, \@{product-rule-for-logarithms}, and \@{conditional-relative-entropy}:

\bal
	D(p(x,y) \| q(x, y)) & = \sum_{x} \sum_{y} p(x,y) \log{\frac{p(x,y)}{q(x,y)}} & & \text{defn. of relative entropy} \\
	                     & = \sum_{x} \sum_{y} p(x,y) \log{\frac{p(x)p(y|x)}{q(x) q(y|x)}} & & \text{multiplication rule} \\
	                     & \text{by the product rule for logarithms:} \\
	                     & = \sum_{x} \sum_{y} p(x,y) \log{\frac{p(x)}{q(x)}} + \sum_{x} \sum_{y} p(x,y) \log{\frac{p(y|x)}{q(y|x)}} \\
	                     & = D(p(x) \| q(x)) + D(p(y|x) \| q(y|x)) & & \text{defns. of relative and conditional relative entropy}
\eal
\end{proof}
...
\begin{corollary}[Nonnegativity of Mutual Information]
	...
	\begin{proof}
		Using the definition of \@{mutual information} and \@{gibbs-inequality}, we have
\bal
...
\begin{proof}
	...
	\bal
	D(p(y|x) \| q(y|x)) & = \sum_{x \in \mathcal{X}} p(x) \sum_{y \in \mathcal{Y}} p(y|x) \log{\frac{p(y|x)}{q(y|x)}} \\
			    & = \sum_{x \in \mathcal{X}} p(x) D(p(\cdot|x) \| q(\cdot|x)). 
	\eal
...
\begin{corollary}[Nonnegativity of Conditional Mutual Information]
	\[ I(X; Y |Z) \geq 0, \]
	...
	\[ I(X; Y | Z = z)  = D(p(x,y |z) \| p(x |z)p(y |z)) \geq 0 \].
...
It remains to show that this extreme of $H$ is a maximum. This follows from the facts that the domain is a \@[convex]{convex-set}, \@{compact} set and that the entropy function is strictly \@{concave}. TODO: more details on these.
\end{proof}
...
% TODO (path B): re-derive from the chain rule and conditioning-reduces-entropy (Cover and Thomas Thm 2.6.6, the independence bound). The current proof below uses Jensen directly.
\end{theorem}
...
\subsection{Log Sum Inequality and Its Applications}
...
\begin{theorem}[Log Sum Inequality]
	...
	We use the conventions that $0 \log{0} = 0, a \log{\frac{a}{0}} = \infty$ if $a > 0,$ and $0\log{\frac{0}{0}} = 0,$ all of which follow from \@{continuity}.
\end{theorem}
\begin{proof}
	We can assume without loss of generality, due to the $0$-handling conventions specified above, that $a_i > 0$ and $b_i > 0.$ Now, note that the function $f(t) = t \log{t}$ is \@{strictly convex}, since $f''(t) = \frac{1}{t} \log{e} > 0$ for all positive $t.$ Then, by \@{jensens-inequality} we have

	\[ \sum \alpha_i f(t_i) \geq f \left ( \sum \alpha_i t_i \right ). \]

	as long as $\alpha_i > 0, \sum_i \alpha_i = 1,$ i.e. if the $\alpha_i$ form a \@{discrete probability distribution}.

	If we let $\alpha_i = \frac{b_i}{\sum_{j=1}^n b_j}$, i.e. $\alpha_i$ is the \@{probability mass} assigned to index $i,$ then this condition is met. We can then pick $t_i = \frac{a_i}{b_i}$ and we get

	\bal
	\sum_{i=1}^n \alpha_i f(t_i) & \geq f \left ( \sum_{i=1}^n \alpha_i t_i \right ) \\ 
	     \iff \sum_{i=1}^n \alpha_i t_i \log{t_i} & \geq \left ( \sum_{i=1}^n \alpha_i t_i \right ) \log{\left ( \sum_{i=1}^n \alpha_i t_i \right )} \\
	     \iff \sum_{i=1}^n \frac{b_i}{\sum_{j=1}^n b_j} \frac{a_i}{b_i} \log{\frac{a_i}{b_i}} & \geq \left ( \sum_{i=1}^n \frac{b_i}{\sum_{j=1}^n b_j} \frac{a_i}{b_i} \right ) \log{\left ( \sum_{i=1}^n \frac{b_i}{\sum_{j=1}^n b_j} \frac{a_i}{b_i} \right )} \\
	     \iff \sum_{i=1}^n \frac{a_i}{\sum_{j=1}^n b_j} \log{\frac{a_i}{b_i}} & \geq \left ( \sum_{i=1}^n \frac{a_i}{\sum_{j=1}^n b_j}  \right ) \log{\left ( \sum_{i=1}^n \frac{a_i}{\sum_{j=1}^n b_j} \right )} \\
	     \iff \frac{1}{\sum_{j=1}^n b_j} \sum_{i=1}^n a_i \log{\frac{a_i}{b_i}} & \geq \frac{1}{\sum_{j=1}^n b_j} \left ( \sum_{i=1}^n a_i  \right ) \log{\left (  \frac{\sum_{i=1}^n a_i}{\sum_{i=1}^n b_j} \right )} \\
	     \iff \sum_{i=1}^n a_i \log{\frac{a_i}{b_i}} & \geq  \left ( \sum_{i=1}^n a_i  \right ) \log{\left (  \frac{\sum_{i=1}^n a_i}{\sum_{i=1}^n b_j} \right )}.
	\eal
...
\end{theorem}
\begin{proof}
	First, note that the set of \@{probability mass functions} on $\mathcal{X}$ is the set of \@{probability vectors} in $\reals^n,$ where $n = |\mathcal{X}|,$ and thus \@[is a convex set]{probability-vectors-form-a-convex-compact-set}. $D$ takes a pair $(p, q) \in \reals^n \times \reals^n,$ which is \@[again convex]{product-of-convex-sets-is-convex}, so $D$ is defined on a \@{convex set}.
	...
	Let
	\bal
	a_1 = \lambda p_1(x), \quad a_2 = (1 - \lambda)p_2(x) \\
	b_1 = \lambda q_1(x), \quad b_2 = (1 - \lambda)q_2(x).
	\eal

	Using those substitutions, with \@{log-sum-inequality}, we have

        \bal
        \left ( \lambda p_1(x) + (1 - \lambda) p_2(x) \right ) \log{\frac{ \lambda p_1(x) + (1 - \lambda) p_2(x) }{ \lambda q_1(x) + (1 - \lambda) q_2(x) }} & =
	\left ( a_1 + a_2 \right ) \log{\frac{ a_1 + a_2 }{ b_1 + b_2 }} \\
	...
	Summing both sides of this over $x \in \mathcal{X}$ gives us
...
\begin{intuition}
	...
	\includegraphics[alt={Convexity of Relative Entropy}]{convexity-of-relative-entropy.png}
	The image above was generated using GPT-5.6 Sol.

	Another way to think of this is that mixing $p_1$ and $p_2$, and $q_1$ and $q_2$ can't make the results more distinguishable than the weighted average distinguishabilities between $p_1$ and $q_1$ and $p_2$ and $q_2$, i.e., mixing distributions tends to wash out distinctions.
\end{intuition}
...
\end{theorem}
\begin{proof}
For the first part, first expand \@{mutual information}:
	...
	is a \@{linear function} of $p(x).$ Since $H(y)$ is a \@{concave function} of $p(y),$ by \@{affine precomposition preserves convexity} it is a concave function of $p(x).$ The second term is a linear function of $p(x),$ so by \@{adding an affine function preserves convexity}, their difference is a concave function of $p(x)$ as well.

	For the second part, fix $p(x)$ and consider two different \@{conditional distributions} $p_1(y|x)$ and $p_2(y|x).$ Their corresponding \@{joint distributions} are $p_1(x,y) = p(x) p_1(y|x)$ and $p_2(x,y) = p(x)p_2(y|x),$ and their respective \@{marginal distributions} are $p(x),p_1(y)$ and $p(x), p_2(y).$ Consider a conditional distribution formed from their \@{convex combination}

	\[ p_\lambda(y|x) = \lambda p_1(y|x) + (1 - \lambda)p_2(y|x). \]

	Their corresponding joint distributions are also a convex combination of the corresponding joint distributions,

	\[ p_\lambda(x,y) = \lambda p_1(x,y) + (1 - \lambda) p_2(x,y), \]

	and the distribution of $Y$ is also a \@{convex combination},

	\[ p_\lambda(y) = \lambda p_1(y) + (1 - \lambda) p_2(y). \]

	Now, if we let $q_\lambda(x,y) = p(x) p_\lambda(y)$ be the product of the marginal distributions, we have

	\[ q_\lambda(x,y) = \lambda q_1(x,y) + (1 - \lambda) q_2(x,y). \]

	The mutual information is the \@{relative entropy} between the joint distribution and the product of the marginals, so

	\[ I(X;Y) = D(p_\lambda(x,y) \| q_\lambda(x,y)), \]

	and relative entropy is a \@{convex function} of $(p, q),$ so the mutual information is a convex function of the conditional distribution.
\end{proof}
...
\end{theorem}
\begin{proof}
	Using \@{Chain Rule for Mutual Information}, we can expand $I(X; Y, Z)$ in two ways:
	...
	Now, $X$ and $Z$ are \@{conditionally independent} given $Y,$ so $I(X; Z|Y) = 0,$ and recalling that $I(X; Y|Z) \geq 0$ we have
	...
	We have equality if and only if $I(X; Y|Z) = 0,$ i.e. if $X \to Z \to Y$ forms a \@{markov chain}.
\end{proof}

\begin{corollary}[Processing Cannot Increase Information]
	If $Z = g(Y),$ we have $I(X; Y) \geq I(X; g(Y)).$
\end{corollary}
\begin{proof}
	From \@{Function of middle variable forms Markov Chain} we have that $X \to Y \to g(Y)$ is a \@{markov chain} and \@{data-processing inequality} gives us $I(X; Y) \geq I(X; g(Y)).$
\end{proof}
\begin{corollary}
	If $X \to Y \to Z,$ then $I(X; Y|Z) \leq I(X; Y).$
\end{corollary}
\begin{proof}
	In \@{proof-of-data-processing inequality} we showed that $I(X; Z|Y) = 0$ and $I(X; Z) \geq 0.$ Then,

	\bal
	I(X;Z) + I(X; Y|Z) & = I(X;Y)  \\
	\iff I(X; Y | Z) & \leq I(X; Y).
	\eal
\end{proof}
...
\begin{definition}[Sufficient Statistic]
	A \@{function} $T(X)$ is said to be a \textbf{sufficient statistic} relative to the family $\{f_\theta(x)\}$ if $X$ is \@{independent} of $\theta$ given $T(X)$ for any \@{probability distribution} on $\theta,$ that is, $\theta \to T(X) \to X$ forms a \@{Markov chain}.
\end{definition}

\begin{theorem}[Characterization of Sufficiency]
	Let $X$ have \@{probability mass function} $f_\theta(x)$ from the family
	$\{f_\theta(x)\}$, and let $T(X)$ be a \@{statistic} of $X$.
	Then $T(X)$ is a \@{sufficient statistic} for $\theta$ if and only if

	\[
		I(\theta;X) = I(\theta;T(X))
	\]

	for every probability distribution on $\theta$.
\end{theorem}
\begin{proof}
	If $I(\theta;X) = I(\theta;T(X))$, then by \@{data-processing-inequality} we have that $\theta \to T(X) \to X$ forms a \@{Markov Chain}, i.e. $T(X)$ is a \@{sufficient statistic} for $\theta.$

	Now, assume $T(X)$ is a \@{sufficient statistic} for $\theta.$ Then again by \@{data-processing-inequality}, $I(\theta;X) = I(\theta;T(X))$.

\end{proof}
\begin{example}
	For a sequence of $n$ coin-flips with parameter $\theta = \Pr{(X_i = \text{heads})},$ the number of heads is a \@{sufficient statistic} for $\theta$, because if we know the number of heads we got, knowing the specific order of heads and tails in the sequence tells us nothing more about $\theta$, as all possible sequences with that many heads are equally likely.
\end{example}
...
\begin{definition}[Probability of Error]
	Given a \@{discrete random variable} $X,$ if $\hat{X}$ is an \@{estimate} of $X,$ then we define the probability of error as
...
\begin{corollary}[Weak Form of Fano's Inequality]
