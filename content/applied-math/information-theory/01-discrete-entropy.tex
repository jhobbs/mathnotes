...
For the first part, first expand \@{mutual information}:
	...
	For the second part, fix $p(x)$ and consider two different \@{conditional distributions} $p_1(y|x)$ and $p_2(y|x).$ Their corresponding \@{joint distributions} are $p_1(x,y) = p(x) p_1(y|x)$ and $p_2(x,y) = p(x)p_2(y|x),$ and their respective \@{marginals} are $p(x),p_1(y)$ and $p(x), p_2(y).$ Consider a conditional distribution formed from their \@{convex combination}
