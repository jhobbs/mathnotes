...
\begin{proof}
	...
	\[ D(p(x,y) || q(x, y))  = \sum_{x} \sum_{y} p(x,y) \log{\frac{p(x,y)}{q(x,y)}}. \]
        ...
        From \@{joint-is-marginal-times-conditional} we have
	\[ \sum_{x} \sum_{y} p(x,y) \log{\frac{p(x,y)}{q(x,y)}} = \sum_{x} \sum_{y} p(x,y) \log{\frac{p(x)p(y|x)}{q(x) q(y|x)}}. \]

	Then, from \@{product-rule-for-logarithms} we have
        \[ \sum_{x} \sum_{y} p(x,y) \log{\frac{p(x)p(y|x)}{q(x) q(y|x)}} = 
	       \sum_{x} \sum_{y} p(x,y) \log{\frac{p(x)}{q(x)}} +
	       \sum_{x} \sum_{y} p(x,y) \log{\frac{p(y|x)}{q(y|x)}}.\]

	Now from the definitions of \@{relative-entropy} and \@{conditional-relative-entropy} we have
	\[     \sum_{x} \sum_{y} p(x,y) \log{\frac{p(x)}{q(x)}} +
	       \sum_{x} \sum_{y} p(x,y) \log{\frac{p(y|x)}{q(y|x)}} = D(p(x)||q(x)) + D(p(y|x)||q(y|x)).
	\]
\end{proof}
