...
\subsection{Elementary Numerical Integration}

The basic method for approximating $\int_a^{b} f(x) dx$ is called \textbf{numerical quadrature} and uses the sum $\sum_{i=0}^n a_i f(x_i).$
...
When we use equally spaced nodes, the approximations produced from first and second Lagrange polynomials are known as the \textbf{Trapezoidal rule} and \textbf{Simpson's rule.} These are commonly introduced in calculus courses.
...
In general, integrating Lagrange interpolating polynomials and there error terms give us \textbf{Newton-Cotes} formulas; the Trapezoidal rule and Simpson's rule are the first two examples of these.
...
For example, the \textbf{Composite Simpson's rule} breaks an interval down into $n$ subintervals for some even $n,$ applies Simpson's rule on each consecutive pair of subintervals, and then sums the results.

\emph{Theorem:} Let $f \in C^4[a,b], n$ be even, $h = (b - a)/n,$ and $x_j = a + jh,$ for each $j = 0, 1, \dots, n.$ There exists a $\mu \in (a, b)$ for which the Composite Simpson's rule for $n$ subintervals can be written with its error term as
...
\emph{Theorem:} Let $f \in C^2[a,b], h = (b - a)/n,$ and $x_j = a + jh$ for each $j = 0, 1, \dots n.$ There exists a $\mu \in (a,b)$ for which the \textbf{Composite Trapezoidal rule} for $n$ subintervals can be written with its error term as

\[ \int_a^{b} f(x) dx = \frac{h}{2} \left [ f(a) + 2 \sum_{j=1}^{n-1} f(x_j) + f(b) \right ] - \frac{b - a}{12} h^2 f''(\mu). \]

For the \textbf{Composite Midpoint Rule} we again require even $n$.

\emph{Theorem:} Let $f \in C^2[a, b],$ $n$ be even, $h = (b - a)/(n + 2)$ and $x_j = a + (j + 1)h$ for each $j = -1, 0, \dots, n+1.$ There exists a $\mu \in (a, b)$ for which the Composite Midpoint rule for $n+2$ subintervals can be written with its error term as
...
We will use a collection of orthogonal polynomials called the \textbf{Legendre Polynomials,} denoted here as $\{P_0(x), P_1(x), \dots, P_n(x)\}$ and having the properties

\begin{enumerate}
\item For each $n$, $P_n(x)$ is a monic polynomial of degree $n.$
\end{enumerate}

\begin{enumerate}
\item $\int_{-1}^{1}P(x)P_n(x)dx = 0$ whenever $P(x)$ is a polynomial of degree less than $n.$
\end{enumerate}
...
\subsubsection{Gaussian Quadrature on Arbitrary Intervals}
