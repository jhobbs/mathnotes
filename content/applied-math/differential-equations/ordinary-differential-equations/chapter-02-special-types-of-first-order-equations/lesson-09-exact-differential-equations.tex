...
\subsection{Review of Integration Theory}

\begin{enumerate}
\item Let $f(x)$ be a function of $x$ defined on an \@{interval} $I: a \leq x \leq b$. Let $I$ be divided into $n$ subintervals and call $\Delta x_k$ the width of the $k$ th subinterval. Then if
\end{enumerate}
...
This limit is called the \textbf{Riemann intergral} of $f(x)$ over $I$. If this limit does not exist, we say $f(x)$ is not Riemann integrable over $I$.

\begin{enumerate}
\item If $f(x)$ is a \@{continuous} funtion of $x$ on an \@{interval} $I: a \leq x \leq b,$ and
\end{enumerate}

\[ \tag{9.13} F(x) = \int_{x_0}^{x} f(u)du, \]

then by the \emph{fundamental theorem of the calculus}
...
\begin{enumerate}
\item Let $P(x,y)$ and $Q(x,y)$ be functions of two independent variables $x,y$ both function being defined on a common domain $D$. If we wish to perform the following integration, the first with respect to $x$ ( $y$ constant), and the second with respect to $y$ ( $x$ constant)
\end{enumerate}
...
\emph{\textbf{Definition 9.19}}
A region is called a \textbf{simply connected region} if \emph{every} simple \emph{closed} curve lying entirely in the region \emph{encloses only points of the region}.
...
\subsection{Definition of an Exact Differential and of an Exact Differential Equation}

\emph{\textbf{Definition 9.23}}  A differential expression

\[ \tag{9.24} P(x,y)dx + Q(x,y)dy \]

is called an \textbf{exact differential} if it is the \@[total differential]{total-derivative} of a function $f(x,y)$, i.e., if

\[ \tag{9.241} P(x,y) = \frac{\partial}{\partial x}f(x,y) \quad \text{and} \quad Q(x,y) = \frac{\partial}{\partial y}f(x,y). \]

\emph{\textbf{Definition 9.27}} The differential equation

\[ \tag{9.28} P(x,y)dx + Q(x,y)dy = 0 \]

is called \textbf{exact} if there exists a function $f(x,y)$ such that its \@{partial-derivative} with respect to $x$ is $P(x,y)$ and its partial derivative with respect to $y$ is $Q(x,y)$. In symbolic notation, the definition says that (9.28) is an \textbf{exact differential equation} if there exists a function $f(x,y)$ such that:
...
\subsection{Necessary and Sufficient Condition for Exactness and Method of Solving an Exact Differential Equation}

\emph{\textbf{Theorem 9.3}} \emph{A necessary and sufficient condition that the differential equation}

\[ \tag{9.31} P(x,y)dx + Q(x,y)dx = 0 \]

\emph{be exact is that}

\[ \tag{9.32} \frac{\partial}{\partial y}P(x,y) = \frac{\partial}{\partial x}Q(x,y) \]

\emph{where the functions defined by P(x,y) and Q(x,y), the \@{partial-derivatives} in (9.32) and} $\partial P(x,y)/\partial x, \partial Q(x,y)/\partial y$ \emph{exist and are \@{continuous} in a simply connected region R}.

\emph{\textbf{Proof of necessary condition}} To summarize an already brief proof, recall from Calculus 3 that the order of partial derivatives doesn't matter, i.e. that $F_xy(x,y) == F_yx(x,y)$. Thus, if such a function $f(x,y)$ whose partial derivatives with respect to $x$ and $y$ are $P(x,y)$ and $Q(x,y)$, respectively, then the second partial derivatives of those functions with respect to $y$ and $x$ must also be equal.

\emph{\textbf{Solution from proof of sufficient condition}} This proof takes several pages in the book and I won't reproduce it here. It does give a method of finding the function which is a solution, and I'll instead talk about how to find that function.
...
\emph{\textbf{Solution from Definition}}

An alternative approach, and one preferred by the authors (and by me), is to instead construct a solution from definition.

\emph{jmh: what follows is my own and has some weird notation I made up and maybe some shakey ideas. The book
...
\emph{jmh: $P_{-1}$ here is my notation for the antiderivative of $P(x,y)$ with respect to x without any constant of integration, since that's what $R(Y)$ is. Not sure what the convention is here. The book shows this by example rather than abstractly.}
...
$P_{-x,y}$ and $Q(x,y)$ will be almost equivalent, with just some constant or function of $y$ left between them, let's call that $W(y)$ - but be clear that $W(y)$ is a \emph{known} function (because both $P_{-x,y}(x,y)$ and $Q(x,y)$ are known, so is their difference where $R'(y)$ is unknown still (as it's the derivative of an unknown arbitrary constant of integration).
