...
\title{Recognizable Exact Differential Equations}

\begin{document}

\section{Recognizable Exact Differential Equations}
...
\emph{jmh} To see this, recognize that the partial derivative wrt to $x$ of 10.1 is the coeffecient of $dx$ and that the partial derivative wrt y is the coefficient of $dy$.

The exact differential equations that are recognizable are called \textbf{integrable combinations}.
...
\section{Finding an Integrating Factor}
...
We consider six possibilities. \emph{jmh: For the first five, the book goes into deriving these $h$ functions but I'm not going to do that here.}

\textbf{1. h is a function only of $x$, i.e. $h = h(x)$}

\[ \tag{10.65} F(x) = \frac{\frac{\partial}{\partial y}P(x,y) - \frac{\partial}{\partial x}Q(x,y)}{Q(x,y)} \]

\[ \tag{10.66} h(x) = e^{\int F(x)dx} \]

\textbf{2. h is a function only of $y$, i.e. $h = h(y)$}

\[ \tag{10.65} G(y) = \frac{\frac{\partial}{\partial x}Q(x,y) - \frac{\partial}{\partial y}P(x,y)}{P(x,y)} \]

\[ \tag{10.66} h(y) = e^{\int G(y)dy} \]

\textbf{3. h is a function of $xy$, i.e. $h = h(u)$, where $u = xy$}
...
\textbf{4. h is a function of $x/y$, i.e. $h = h(u)$, where $u = x/y$}
...
\textbf{5. h is a function of $y/x$, i.e. $h = h(u)$, where $u = y/x$}
...
\textbf{6. Special form with matching exponents}

\emph{jmh: The book doesn't give this form a name and doesn't consider it as one of the "5 methods." It may be that it ends up working out via the above methods but is a different approach; I'm not sure.}
...
\emph{jmh: Note that in (10.82), the exponents of} $x$ \emph{and} $y$ \emph{must be the same in both coefficients and that} $dx$\emph{'s coefficient must have a factor of} $y$ \emph{and} $dy$\emph{'s coefficient must have a factor of} $x$\emph{.}
...
\emph{jmh: that, is if} $\frac{\partial}{\partial y}P(x,y) = \frac{\partial}{\partial x}Q(x,y)$.
...
\subsection{Note on finding integrating factors in the exercises}
\emph{jmh} A few of the excercises have solutions that don't seem to fit in to any of the forms above. Googling, I've found that ways to find these types of integrating factors exist, but again don't match the methods in this lesson.
