...
\title{Summary of First Order Differential Equations}

\begin{document}

\section{Summary of First Order Differential Equations}
...
\begin{itemize}
\item \textbf{Separable} - can be separated so that $P(x,y)$ a function of only $x$ and $Q(x,y)$ is a function of only $y$
\item \textbf{Homogenous Coffecients} - $P(x,y)$ and $Q(x,y)$ are homogenous, i.e. they have a common factor $x^n$ or $y^n$. These can be made separable by substituting $y = ux$ or $x = uy$.
\item \textbf{Linear Coefficients} - $P(x,y)$ and $Q(x,y)$ are linear. These can be solved through translation of axes when non-parallel. When parallel, they can be solved by substituting $u = P(x,y),~v = Q(x,y)$, which will lead to an equation with homgenous coefficients.
\item \textbf{Exact Differential Equations} - These have the property that there is a function $f(x,y)$ such that $\frac{\partial f(x,y)}{\partial x} = P(x,y),~\frac{\partial f(x,y)}{\partial y} = Q(x,y).$ These can be solved from this definition, and identified and also solved by using the fact that $\frac{\partial}{\partial y}P(x,y) = \frac{\partial}{\partial x}Q(x,y)$.
\item \textbf{Integrable Combinations} - We can sometimes recognize pairs of differential coefficients as integrable combinations, where they are the parts of a \@[total differential]{total-derivative}. Sometimes a differential equation can have more than one pair of integrable combinations.
\item \textbf{Inexact Differential Equations} - Sometimes we can make an inexact differential equation exact by finding an integrating factor. There are a few special cases we can find this in - see the notes for details.
\end{itemize}

Another important form is the \emph{Linear differential equation}, which can be written like this:

\[ \tag{b} \frac{dy}{dx} + P(x)y = Q(x) \]

\begin{itemize}
\item \textbf{Linear Differential Equation} - Both the dependent variable (usually $y$ but can be $x$) and its derivative are of the first degree. These can be solved easily by letting $h(x) = e^{\int{P(x)}dx}$ and using the fact that $h(x)y = \int{h(x)Q(x)}dx + c$.
\item \textbf{Bernoulli Equation} - can be written as $\tag{b} \frac{dy}{dx} + P(x)y = Q(x)y^n$. This is not linear but can be made linear by multiplying by $(1-n)y^{-n}$.
\item \textbf{Ricatti Equation} - An equation of the form $y' = f_0(x) + f_1(x)y + f_2(x)y^2,~f_2(x) \neq 0$. If $y_1(x)$ is a particular solution, we can make a substitution to transform the equation into a first order linear equation.
\end{itemize}
...
\subsection{A note on substitutions}
...
\section{General Approach to Solving}
