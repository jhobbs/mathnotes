...
\title{Linear Independence of Functions. The Linear Differential Equation of Order n.}
...
\section{Linear Independence of Functions. The Linear Differential Equation of Order n.}

\subsection{Linear Independence of a Function}

A set of functions $f_1(x), f_2(x),\cdots,f_n(x),$ each defined on a common interval $I,$ is called \textbf{linearly dependent} on $I,$ if there exists a set of constants $c_1,c_2,\cdots,c_n,$ not \emph{all} zero, such that:

\[ \tag{19.11} c_1f_1(x) + c_2f_2(x) + \cdots + c_nf_n(x) = 0 \]

for \emph{every} $x$ in $I$. If no such set of constants $c_1,c_2,\cdots,c_n,$ exists, then the set of functions is called \textbf{linearly independent}.

The left side of $(19.11)$ is called a \textbf{linear combination} of the set of functions $f_1(x), f_2(x),\cdots,f_n(x).$

\emph{jmh}: There are plenty of examples of sets of linearly dependent and independent functions elsewhere.
...
The book suggests there are ways to test for linear independence but they aren't taught until the very end of the book. \emph{end jmh}

\subsubsection{Background from  18.1:}

\textbf{A linear differential equation of order n} is an equation which can be written in the form
...
If $Q(x) \not\equiv 0$ on $I,$ $(18.11)$ is called a \textbf{nonhomogenous linear differential equation of order n}.

If in $(18.11)$ $Q(x) \equiv 0$ on $I$, the resulting equation:

\[ \tag{18.13} f_n(x)y^{(n)} + f_{(n-1)}(x)y^{(n-1)} + \cdots + f_1(x)y' + f_0(x)y = 0 \]

is called a \textbf{homogenous linear differential equation of order n.}

Note: this is a different use of the term homogenous than in Lesson 7.

\subsubsection{Linear Independence and Linear Differential Equations of Order n}

\emph{\textbf{Theorem}} A homogenous linear differential equation has as many linearly independent solutions as the order of its equation. See Lesson 65.4 for proof.

\subsection{The Linear Differential Equation of Order $n$}

\emph{\textbf{Theorem 19.2}} If $f_0(x),f_1(x),\cdots,f_n(x),$ and $Q(x)$ are each \@{continuous} functions of $x$ defined on a common interval $I$, and $f_n(x) \neq 0$ when $x$ is in $I$, then the linear diferential equation
...
This is an \emph{existence and uniqueness theorem}. This is a very important theorem, but its proof would be too much to get into now, and is delayed to Lesson 65.

\emph{\textbf{Theorem 19.3}} If $f_0(x),f_1(x),\cdots,f_n(x),$ and $Q(x)$ are each \@{continuous} functions of $x$ defined on a common interval $I$, and $f_n(x) \neq 0$ when $x$ is in $I$, then
...
\subsection{A Few Proofs from Exercises}

\emph{\textbf{Theorem from Exercise 5}}: If $y_p$ is a solution of:

\[ \tag{19.5} f_n(x)y^{(n)} + \cdots + f_1(x)y' + f_0(x)y = Q(x) \]

then $Ay_p$ is a solution of (19.5) with $Q(x)$ replaced by $AQ(x).$

\emph{\textbf{Proof}}: By hypothesis, $y_p$ is a solution of (19.5), that is:
...
\emph{\textbf{Theorem from Exercise 6: Principle of Superposition}}: If $y_{p1}$ is a solution of (19.5) with $Q(x)$ replaced by $Q_1(x)$ and $y_{p2}$ is a solution of (19.5) with $Q(x)$ replaced by $Q_2(x)$, then $y_p = y_{p1} + y_{p2}$ is a solution of:

\[ \tag{19.5d} f_n(x)y^{(n)} + \cdots + f_1(x)y' + f_0(x)y = Q_1(x) + Q_2(x) \]

\emph{\textbf{Proof}}: By hypothesis:
...
\emph{\textbf{Theorem from Exercise 7}}: If $y_p(x) = u(x) + iv(x)$ is a solution of 
...
Rearranging terms in $(19.5m)$ and factoring out $i$ lets us rewrite the left-hand side in real and imaginary parts (see notes on \href{/mathnotes/analysis/complex-analysis/module-01-algebriac-properties/}{Complex Numbers}):
