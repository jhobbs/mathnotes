...
\title{The Differential Equation}
...
\emph{\textbf{Definition 3.1 - Ordinary Differential Equation}}

Let $f(x)$ define a function of $x$ on an interval $I: a < x < b$. By an \textbf{ordinary differential equation} we mean an equation involving $x$, the function $f(x)$ and one or more of its derivatives.

\textbf{Note.} It's customary to replace $f(x)$ by $y$.

\emph{\textbf{Definition 3.2 - Order of a Differential Equation}}
...
\emph{\textbf{Definition 3.6 - Implicit Solution}}
...
\begin{enumerate}
\item it defines $y$ as an implict function of $x$ on $I$, i.e. if there exists a function $g(x)$ defined on $I$ such that $f[x,g(x)] = 0$ for \emph{every} $x$ in $I$, and if
\item $g(x)$ satisfies 3.61, i.e. if
\end{enumerate}

\[ F[x,g(x),g'(x),\cdots,g^{(n)}(x)] = 0 \]

for \emph{every} $x$ in $I$.
