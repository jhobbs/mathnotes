...
\title{Summary of Methods of Solving Higher Order Linear Differential Equations}

\begin{document}

\section{Summary of Methods of Solving Higher Order Linear Differential Equations}

\emph{jmh} These are my notes on how to use the methods of Chatpers 4 and 5 from the book.

\subsection{Homogenous Linear Differential Equations}
...
Find the complementary function $y_c$ by \pagelink[solving the characteristic equation]{applied-math/differential-equations/ordinary-differential-equations/chapter-04-linear-differential-equations-of-order-greater-than-one/lesson-20-solution-of-the-homogeneous-linear-differential-equation-of-order-n-with-constant-coefficients}

\subsubsection{Nonconstant Coefficients}

For equations of the form:

\[ f_n(x)y^{(n)} + f_{(n-1)}(x)y^{(n-1)} + \cdots + f_1(x)y' + f_0(x)y = 0, \]

Follow the method of \pagelink[reduction of order]{applied-math/differential-equations/ordinary-differential-equations/chapter-04-linear-differential-equations-of-order-greater-than-one/lesson-23-solution-of-the-linear-differential-equation-with-nonconstant-coefficients-reduction-of-order-method}.

\subsubsection{Nonhomogenous Linear Differential Equations}
...
\paragraph{Method of Reduction of Order}

For equations of the form

\[ f_n(x)y^{(n)} + f_{(n-1)}(x)y^{(n-1)} + \cdots + f_1(x)y' + f_0(x)y = Q(x), \]

where the coefficients and $Q(x)$ are continuous functions of $x$ on a common interval $I$ and $f_n(x) \neq 0$, we can use the method of \pagelink[reduction of order]{applied-math/differential-equations/ordinary-differential-equations/chapter-04-linear-differential-equations-of-order-greater-than-one/lesson-23-solution-of-the-linear-differential-equation-with-nonconstant-coefficients-reduction-of-order-method}.
...
\paragraph{Solution by Inverse Differential Operators}

We can use \pagelink[Inverse Differential Operators]{applied-math/differential-equations/ordinary-differential-equations/chapter-05-operators-and-laplace-transforms/lesson-25-inverse-operators} to find a particular solution under the same conditions that apply for the method of undetermined coefficients (constant coefficients, finitely many linearly independent derivatives of $Q(x)$. This method can often be simpler than other methods for finding particular solutions.

See also \pagelink[Partial Fraction Decomposition of Inverse Operators]{applied-math/differential-equations/ordinary-differential-equations/chapter-05-operators-and-laplace-transforms/lesson-26-solution-of-a-linear-differential-equation-by-means-of-the-partial-fraction-expansion-of-inverse-operators}.

\paragraph{Note on shifted sin or cos in Q(x)}

(\emph{jmh}) I believe and have used this fact but haven't proven it. When $Q(x) = b\sin{(ax+\delta)}$, we can treat it like $b\sin{(ax)}$ and solve using methods like that and then add the $\delta$ term back into $\sin$ and $\cos$ functions in the solution. My reasoning is that $\delta$ is just a horizontal shift of the forcing function. The shift doesn't change the essential nature of its solutions, because each derivative of $Q(x)$ has the same shift, and the shift never comes out of the $\sin$ function to show up other places in the solution. The same thing applies for $b\cos{(ax+\delta)}$ in $Q(x)$.

\paragraph{Solution by Laplace Transforms}

We can use \pagelink[Laplace Transforms]{applied-math/differential-equations/ordinary-differential-equations/chapter-05-operators-and-laplace-transforms/lesson-27-the-laplace-transform-and-gamma-function} to solve differential equations given initial conditions.
