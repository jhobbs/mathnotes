...
\title{The Laplace Transform. Gamma Function.}
...
\section{The Laplace Transform. Gamma Function.}

\subsection{Definition of the Laplace Transform}

\textbf{Definition 27.13} Let $f(x)$ be defined on the interval $I: 0 \leq x \lt \infty$. Then the \textbf{Laplace Transform of} $f(x)$, written as $L[f(x)]$, is defined as
...
\textbf{Definition 27.18} If $F$ is the Laplace transform of a \emph{continuous} function $f$, i.e. if:

\[ L[f] = F \]

then the \textbf{inverse Laplace transform} of $F$, written as $L^{-1}[F]$ is $f$, i.e.:
...
See Paul's notes for an excellent \href{https://tutorial.math.lamar.edu/classes/de/laplace_table.aspx}{Table of Laplace Transforms}

\subsection{Solution by Means of a Laplace Transform}

Given a linear equation:

\[ \tag{27.2} a_n y^{(n)} + \cdots + a_1 y' + a_0y = f(x) \]

where $ a_n \neq 0$ and $a_0, a_1, \cdots, a_n$ are constants, we can use the \textbf{Laplace transform method} to find a particular solution \emph{satisfying given initial conditions}. This is what makes the Laplace transform method useful in compared to the other methods we've studied for solving linear differential equations.
...
(\emph{jmh} my notes, solution to 28c,3)
...
Now we can look for similar transforms in a table. From \href{https://tutorial.math.lamar.edu/classes/de/laplace_table.aspx}{Table of Laplace Transforms} we'll want to use these two identities:
...
We can use these properties along with a \href{https://en.wikipedia.org/wiki/Particular_values_of_the_gamma_function}{table of Gamma function values} to find other values of Gamma quickly.
