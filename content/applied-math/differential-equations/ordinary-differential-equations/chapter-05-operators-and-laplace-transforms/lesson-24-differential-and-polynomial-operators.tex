...
\title{Differential and Polynomial Operators}
...
\subsection{Definition of an Operator. Linear Property of Polynomial Operators}
...
By convention, the letter $D$ is used to denote the operation of differentiation and is called a \textbf{differential operator}.
...
Because of the resemblance of $P(D)$ to a polynomial, we shall refer to it as a \textbf{polynomial operator of order n}.

\textbf{Definition 24.13} Let $P(D)$ be the polynomial operator (24.12) of order $n$ and let $y$ be an $n$th order differntiable function. Then we define \textbf{P(D)y} to mean:
...
\textbf{Theorem 24.2} \emph{If} $P(D)$ \emph{is the polynomial operator} $(24.12)$ \emph{and} $y_1,~y_2$ \emph{are two nth order differentiable functions, then}

\[ \tag{24.21} P(D)(c_1 y_1 + c_2 y_2) = c_1 P(D) y_1 + c_2 P(D) y_2, \]

\emph{where} $c_1$ \emph{and} $c_2$ \emph{are constants.} 

(\emph{jmh}: See the book for proof, but it's just distributive property and the sum rule for derivatives.)

An operator which has the property $(24.21)$ is called a \textbf{linear operator}. Hence the polynomial operator $(24.12)$ is \textbf{linear}.
