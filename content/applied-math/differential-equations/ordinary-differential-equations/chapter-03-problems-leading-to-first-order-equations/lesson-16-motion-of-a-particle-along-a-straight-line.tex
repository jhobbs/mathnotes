...
\title{Motion of a Particle Along a Straight Line}
...
\section{Motion of a Particle Along a Straight Line}
...
The proportionality constant $G$ is called the \textbf{gravitational constant}. We use the negative sign because the resultant force is downward towards the earth's surface, and here we're considering our positive direction to be upward.

\subsubsection{Vertical Motion with no Air Resistance and Close to the Surface of the Earth}
...
The constant $g$ is thus the acceleration of a body due to the earth's \textbf{force of gravity}. Its value varies slightly for different locations on earth and for different heights, but for convenience we'll use the value $32 ft/sec^2$.
...
(\emph{jmh: my notes}) One interesting thing from the book that isn't covered in the blog post above is finding velocity as a function of position.
...
\subsubsection{Far from the Surface of the Earth}
...
\paragraph{Velocity as a Function of Distance}

Velocity of a function of distance is interesting for at least two cases:

\begin{itemize}
\item For falling objects, finding the velocity when $r = 0$, i.e. maximum impact velocity, setting air resistance aside.
\item For rising objects, setting velocity to $0$ finds the maximum distance an object will travel from the surface.
\end{itemize}
...
In horizontal motion, we have friction between a moving object and the surface it's moving on. We call this \textbf{sliding friction} to distinguish it from the friction a stationary object has (\emph{static friction}).

The amount of sliding friction for a given object moving on a given surface is determined by two factors:

\begin{enumerate}
\item The gravitational force pressing the object into the surface ($mg$).
\item The smoothness/roughness particular the object and surface and is given as the constant $\mu$ called the \textbf{coefficient of friction}.
\end{enumerate}
...
\emph{jmh}: Horizontal motion is pretty easy and boring compared to vertical motion.

\subsection{Inclined Motion}
...
\begin{enumerate}
\item The acceleration due to gravity. This is equal to $\omega\sin{\alpha}$ - that is, the vertical component of the slope multiplied times the weight of the object. This make sense - consider the case where $\alpha$ is $90\degree$; $\sin{90\degree} = 1$ so we get the full weight ($m\times g$) of the object.
\item The drag due to friction. Letting $\mu$ be the coefficient of friction, and the downward direction as positive, this is equal to $-\mu\omega\cos{\alpha}$; that is, the horizontal component of the slope multipled times the weight of the object and the coefficient of friction. For intuition, consider when $\alpha = 0\degree$. $\cos{0\degree} = 1$ so we get the full force of friction. As the angle increase, the friction decreases, which is why if you tip a table over, eventually the stuff sitting on it will slide off.
\end{enumerate}
