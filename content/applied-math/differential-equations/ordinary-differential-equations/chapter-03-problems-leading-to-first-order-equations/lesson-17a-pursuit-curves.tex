...
\subsection{Bug on a Turntable Problems}
...
\subsubsection{4. Bug Walking Towards The Center}
...
\subsubsection{5. Bug Walking Parallel to the $x$-axis}
...
Using formulas from my \href{https://jhobbs.github.io/mathnotes/calculus/polar-coordinates.html}{polar coordinates notes} (see the 'Derivatives' section here ) we can find $r'$ and $\theta'$:
...
\subsubsection{12. Bug Walking Towards a Light}
...
Using formulas from my \href{https://jhobbs.github.io/mathnotes/calculus/polar-coordinates.html}{polar coordinates notes} (see the 'Radial and Transverse Components of Motion' section here), we can find that the differential equations capturing the locomotive motion of the bug are:
...
\begin{itemize}
\item This uses Euler's method to estimate the paths rather than using exact solutions.
\item Instead of assuming the bug always enters the turntable at the $x$-axis, we allow the bug to start anywhere.
\item We're a little hazy with the units of speed and distance. Animation is hard. I'd like to improve this.
\end{itemize}

We show a limited history of the bug's position to make the path more obvious. There are also 3 \@{vectors} drawn:

\begin{itemize}
\item blue - the bug's locomotive velocity
\item red - the bug's linear velocity from the turntable's rotation
\item yellow - the bug's combined velocity from its locomotive motion and the turntable's rotation
\end{itemize}

\includedemo{turntable}

It's interesting to me that the "always walk towards the light" approach can wind up with the bug never reaching the light, but instead getting stuck at some kind of \@[equillibrium]{equilibrium} point where the bug's locomotive velocity and the linear velocity from the rotation of the turntable are equal in value and opposite in direction. I wonder how we find which initial conditions lead to the bug reaching the light and which don't? For those that don't, I wonder how we find the equillibrium point the bug will stop at.
