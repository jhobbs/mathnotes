...
\title{Miscellaneous Types of Problems Leading to Equations of the First Order}

\begin{document}

\section{Miscellaneous Types of Problems Leading to Equations of the First Order}

\subsection{First Order Linear Electric Circuit}

Take a circuit where an applied electromotive force $E$, an inductor $L$ and a resistor are connected in series:

\includegraphics[alt={ELR Circuit}]{/mathnotes/applied-math/differential-equations/ordinary-differential-equations/chapter-03-problems-leading-to-first-order-equations/elr-circuit.jpg}
...
This \href{https://www.desmos.com/calculator/tzigt2uc7x}{desmos link} has graphs of the solutions when the emf is constant (DC) or varying sinusoidally with time (AC).  

Some interesting things to note:

\begin{itemize}
\item For constant $E$, $L$ and $R$:
\item We get a steady state current when the current grows enough that current loss caused by resistance equals the current growth caused by emf.
\item This is makes the steady state current a sort of "terminal current" just like terminal velocity is the steady state where acceleration from gravity equals deceleration from air resistance.
\item Increasing resistance reduces the peak current for both the DC and AC scenarios
\item Increasing inductance decreases the rate of change of current for both DC and AC scenarios.
\begin{itemize}
\item For DC, this slows the ramp up/ramp down to the steady state current, which is $\frac{E}{R}$.
\item For the AC circuit, the slower current change that comes from increasing inductance has the effect of reducing the peak current. This makes sense since the current is changing more slowly, it has less time to change before the voltage alternates and starts pushing the current back in the other direction.
\item The lag in current change caused by the inductance brings the current out of phase with the emf. This is what results in the lower peak currents and is known as inductive reactance. Increasing inductance you can see that the current ends up being a quarter of a cycle behind the voltage. 
\end{itemize}
\item Decreasing inductance to near zero makes the DC circuit behave closer and closer to the ideal of Ohm's law: $i = \frac{E}{R}$
\end{itemize}

\subsection{Chain Hung over Frictionless Support}
...
\includegraphics[alt={Chain force}]{/mathnotes/applied-math/differential-equations/ordinary-differential-equations/chapter-03-problems-leading-to-first-order-equations/chain-force.jpg}
...
\subsection{Variable Mass Rocket}
...
\begin{itemize}
\item $m$ - mass of the rocket at time $t$
\item $v$ - velocity of the rocket at time $t$
\item $F$ - sum of all the forces acting on the rocket at time $t$; usually gravity and sometimes drag
\item $dm$ - mass leaving the rocket in time interval $dt$
\item $u$ - velocity of $dm$ at the moment it leaves the rocket, relative to the rocket
\end{itemize}

The mass of the rocket at time $t$ can be given as $m = M + m_0 - kt$ where the values mean the following:

\begin{itemize}
\item $M$ - the fixed mass of the structure of the rocket
\item $m_0$ - the initial mass of the fuel of the rocket
\item $kt$ - the mass of the fuel burned by time $t$
\end{itemize}
...
MIT has a great \href{https://ocw.mit.edu/courses/16-07-dynamics-fall-2009/98d19cb2b9983c14c6907052499e7a98_MIT16_07F09_Lec14.pdf}{write up on the variable mass rocket equation}.
