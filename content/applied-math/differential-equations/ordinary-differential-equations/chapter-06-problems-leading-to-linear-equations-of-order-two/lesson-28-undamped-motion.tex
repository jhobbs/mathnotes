...
\section{Undamped Motion}

\subsection{Free Undamped Motion (Simple Harmonic Motion)}
...
\emph{\textbf{Definition 28.1}} A particle will be said to execute \textbf{simple harmonic motion} if its equation of motion satisfies a differential equation of the form:
...
The solution of $(28.11)$ is (see \pagelink[Solution of the Homogeneous Linear Differential Equation of Order n with Constant Coefficients]{applied-math/differential-equations/ordinary-differential-equations/chapter-04-linear-differential-equations-of-order-greater-than-one/lesson-20-solution-of-the-homogeneous-linear-differential-equation-of-order-n-with-constant-coefficients}):
...
\textbf{Simple harmonic motion} is the motion of a particle whose position $x$ as a function of time $t$ is given by any of the three equations above (or any other equivalent equations).
...
\textbf{Some Important Parameters}

(\emph{jmh} my notes)

First, review my notes on \pagelink[Frequency and Period]{geometry/trigonometry/frequency}.
...
\begin{itemize}
\item $\|c\|$ - the amplitude of motion, i.e., the maximum distance of the particle from the origin.
\item $\| c \omega_0 \|$ - the maximum velocity of the particle.
\item $x = 0$ - the equilibrium point, i.e. the center point of motion.
\item $T = \frac{2 \pi}{\omega_0}$ - the period of the motion, i.e. the amount of time it takes the particle to make one complete oscillation about its equilibrium point
...
\item $\delta$ - the phase angle of $x$. From $(d)$ above, when $t = 0$, $x = c \cos{\delta}$, i.e. $c \cos{\delta}$ gives the starting position of the particle.
\item $\omega$ - the impressed angular frequency of the system (angular frequency of forcing function)
\item $\omega_0$ - the natural (undamped) angular frequency of the system
\end{itemize}

\subsubsection{Examples of Simple Harmonic Motion}

\paragraph{The Motion of a Particle Attached to an Elastic Helical Spring. Hooke's Law}
...
where k is a proportionality constant called the \emph{spring constant} or \emph{stiffness coefficient} of the spring.
...
\begin{enumerate}
\item An upward force due to the tension o the spring, which by Hooke's law is $k(l + y)$.
\item A downward force due to the weight, which is $mg$.
\end{enumerate}
...
\begin{itemize}
\item Amplitude: $\|c\|$, because we stretch the spring $c$ beyond its equilibrium point.
\item Maximum velocity: $\|c \sqrt{\frac{k}{m}}\|$
\item Equilibrium point: $y = 0$
\item Period: Note that ${\omega_0}^2 = \frac{k}{m}$. The period is then $\frac{2 \pi}{\frac{k}{m}} = 2 \pi \sqrt{\frac{m}{k}}$.
\item Natural Frequency: $\nu = \frac{\sqrt{k}}{2 \pi \sqrt{m}} \text{cps}$
\item Phase Angle: Will be zero in the case the spring is simply released from the maximum distance, but in any case, can be found by solving $y = c \cos{\delta}$ at time $t = 0$.
\end{itemize}

\paragraph{The Motion of a Simple Pendulum}
...
\subparagraph{With Small Angle Approximation}
...
\begin{itemize}
\item Position of the bob as a function of time:
\end{itemize}
...
\begin{itemize}
\item Angular velocity of the bob as a function of time:
\end{itemize}
...
\begin{itemize}
\item Linear velocity of the bob as a function of time:
\end{itemize}
...
\begin{itemize}
\item Amplitude: $A = \sqrt{ {\theta_0}^2 + \frac{l}{g}{ \omega_0}^2} $ ft
\item Period: $T = 2 \pi \sqrt{\frac{l}{g}}$ sec
\item Natural Frequency: $\nu = \frac{1}{2 \pi} \sqrt{gl} $ cps
\item Phase Angle: $\delta =  - \arctan2{( \omega_0 \sqrt{\frac{l}{g}}, \theta_0)} $ rad
\end{itemize}
...
\begin{itemize}
\item Amplitude: $A = \omega_0 $ rad
\item Period: $T = 2 \pi \sqrt{\frac{l}{g}}$ sec
\item Natural Frequency: $\nu = \frac{1}{2 \pi} \sqrt{gl} $ cps
\item Phase Angle: $\delta =  0$ rad
\end{itemize}
...
\subparagraph{Without Small Angle Approximation}
...
\subsection{Forced Undamped Motion}
...
Here, $f(t)$ is called the \textbf{forcing function}.
...
A particular solution $y_p$ of $(28.81)$ then depends on the relative values of the natraul (undamped) frequency $\omega_0$ of the system and the \textbf{impressed frequency} $\omega$ of the forcing function $mF\sin{(\omega t + \beta)}$.
...
I've made a \href{https://www.desmos.com/calculator/caqdrbowgi}{Desmos graph of this general solution}.
...
Now we have that the maximum displacement of the particle is $\|c\| + \left \| \frac{F}{2 \omega_0} t \right \| $. The presence of the variable $t$ in the second term implies that the displacement due to this part of the motion increases with time. A motion in which the displacement increases beyond all bounds as time passes is called an \textbf{unstable motion}. In such cases, a mechanical breakdown of the system is bound to occur. This condition is known as \textbf{undamped resonance} and $\omega_0$ is called the \textbf{undamped resonant frequency}.

Here is a \href{https://www.desmos.com/calculator/6a4rofhsfe}{Desmos graph of this general solution}.

Note that in engineering circles, $f(t)$ in $(28.8)$ is called the \textbf{input} of the system and the solution $y(t)$ is called the \textbf{output} of the system.

\subsubsection{Some Intresting Examples of Forced Undamped Motion}
