...
\subsection{Simple Electric Circuits}

Given a circuit with:

\begin{itemize}
\item An energy source providing emf (electromotive force) measured in volts
\item A resistor with resistance \textbf{R} measured in ohms, with a voltage drop of $Ri$
\item An inductor with coefficient of inductance \textbf{L} measured in henrys, with a voltage drop of $L\frac{di}{dt}$
\item A capacitor with capacitance \textbf{C} measured in farads, with a voltage drop of $\frac{1}{C}q$
\end{itemize}

connected in series, we have a charge \textbf{q} masured in columbs, and a current $i = dq/dt$ measured in amperes.
