...
\textbf{Periodic Functions}: A function is \textbf{periodic of period} $T$ if $f(x + T) = f(x)$ for all $x$ in the \@[domain]{domain-of-definition} of $f$.

\textbf{Even Functions}: A function that satisfies $f(-x) = f(x)$ for all $x$ in the domain of $f$ has a graph that is symmetric with respect to the $y$-axis. We say such a function is \textbf{even}. The functions $1, x^2, x^4,\dots$ are examples of even functions, as is $\cos{x}$.

\textbf{Odd Functions}: A function $f$ that satisfies $f(-x) = -f(x)$ for all $x$ in the domain of $f$ has a graph that is symmetric with respect to the origin. It is said to be an \textbf{odd} function. The functions $x, x^3, x^5, \dots$ are examples of odd functions, as are $\sin{x}$ and $\tan{x}$.
...
\includegraphics[alt={Summary of important integrals}]{/mathnotes/applied-math/differential-equations/partial-differential-equations/fsints.png}

\section{Fourier Series}

\textbf{Definition}: Let $f$ be a piecewise continuous function on the \@{interval} $[-L, L]$. The \textbf{Fourier Series} of $f$ is the trigonometric series
...
Formulas (9) and (10) are called the \textbf{Euler-Fourier formulas}. The $\sim$ symbols in (8) means that the series is associated with $f(x)$ but may not converge to $f(x)$.
...
Let $f(x)$ be a piecewise continuous function on the interval $[0, L].$ The \textbf{Fourer cosine series} of $f(x)$ on $[0, L]$ is
...
The \textbf{Fourer sine series} of $f(x)$ on $[0, L]$ is
...
The series in (4) is the Fourier series for $f_e(x)$, and that in (6) is the series for $f_o(x)$. These are called \textbf{half-range expansions} for $f(x)$.
