...
Let $X$ be a \@{random-variable} with a probability distribution $f(x).$ The \textbf{mean} or \textbf{expected value} of $X$ is
...
Let $X$ be a \@{random-variable} with probability distribution $f(x)$ and mean $\mu.$ The \textbf{variance} of $X$ is
...
\begin{intuition}\label{variance-interpretation}
...
Let $X$ be a random variable with finite \@{expectation} $E[X]$, and let $\varphi : \reals \to \reals$ be
a \@{convex-function}. Then

\[ \varphi\big(E[X]\big) \leq E\big[\varphi(X)\big]. \tag{a} \]

If $\varphi$ is \@[concave]{concave-function}, the inequality reverses:
