...
\begin{definition}[Expected Value]\synonyms{"expectation"}
Let $X$ be a \dref{random-variable} with a probability distribution $f(x).$ The \textbf{mean} or \textbf{expected value} of $X$ is

\[ \mu = E(X) = \sum_{x}{x f(x)} \]

if $X$ is \dref{discrete}, and

\[ \mu = E(X) = \int_{-\infty}^{\infty}{x f(x) dx} \]

if $X$ is \dref{continuous}.
...
Let $X$ be a \dref{random-variable} with probability distribution $f(x)$ and mean $\mu.$ The \textbf{variance} of $X$ is
...
The positive square root of the \dref{variance}, $\sigma,$ is called the \textbf{standard deviation} of $X.$
\end{definition}

Variance and standard deviation tell us about how spread out the values of $X$ are around its mean.

\begin{theorem}[Jensen's Inequality]\label{jensens-inequality}
Let $X$ be a random variable with finite \dref{expectation} $E[X]$, and let $\varphi : \mathbb{R} \to \mathbb{R}$ be
a \dref{convex-function}. Then
...
In either case, equality holds if and only if $\varphi$ is \dref{affine} on the support of $X$, or $X$ is almost surely constant.
\end{theorem}
...
The last line is just the definition of a \dref{convex-function}, so it's obviously true.
...
\includedemo{jensens-inequality}
