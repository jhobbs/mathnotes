...
\source{title={CS 6815: Pseudorandomness and Combinatorial Constructions}, author=Eshan Chattopadhyay, type={lecture notes}, section={Lecture 3 (Aug 30, 2022)}, published=2022, url={https://courses.cs.cornell.edu/cs6815/2022fa/lec3.pdf}, accessed=2026-07-22, notes={Cornell University; scribed by Noam Ringach}}
...
\begin{theorem}[Markov's Inequality]\label{markovs-inequality}
Let $X$ be a non-negative random variable ($X \geq 0,$) and $t > 0.$ Then,
...
Now, $E[X \mid X \ge t]$ is the \@{expected-value} when $X \ge t,$ which is necessarily at least $t,$ since $X$ is restricted to taking on values of at least $t.$ So, we have that $E[X \mid X \ge t] \ge t.$
...
\begin{example}\label{markov-inequality-fish}
...
By \@{Markovs-inequality}, for any nonnegative random variable $X$ and any $t > 0$,
...
\begin{theorem}[Chebyshev's Inequality]\label{chebyshevs-inequality}
Suppose $t > 0$ and $X$ is a random variable with the finite mean $E[X]$ and \@{standard-deviation} $\sigma.$ Then
...
Also note that while \@{Markovs-inequality} only requires knowledge of the mean $E[X]$ (the first moment), Chebyshev's inequality requires knowledge of the variance $E[X^2] - E[X]^2$ (second moment). Also, Chebyshev's inequality works for any random variable with a variance defined, rather than just on non-negative random variables.
\end{note}
...
By \@{Markovs-inequality}, for any $a > 0$, we have
...
\begin{example}\label{chebyshev-fish}
...
\@{Chebyshevs-inequality} states that for any $t > 0$:
...
\begin{definition}[Pairwise Independence]\synonyms{"pairwise independent"}
        \@{Random variables} $X_1, \dots, X_n$ are said to be \textbf{pairwise independent} if $X_i$ and $X_j$ are \@{independent} for every $\{i,j\} \in \binom{[n]}{2}$, that is, if
        \[ F_{X_i, X_j}(x, y) = F_{X_i}(x) F_{X_j}(y) \]
        for all $x, y \in \mathbb{R}$, where $F_{X_i, X_j}(x,y) = P(X_i \leq x, X_j \leq y)$ is the joint \@{cumulative distribution function} of the pair.

        \@{Events} $A_1, \dots, A_n$ are said to be \textbf{pairwise independent} if
        \[ P(A_i \cap A_j) = P(A_i) P(A_j) \]
        for every $\{i,j\} \in \binom{[n]}{2}$.
\end{definition}

\begin{definition}[Mutual Independence]\synonyms{"mutually independent"}
        \@{Random variables} $X_1, \dots, X_n$ are said to be \textbf{mutually independent} if their joint \@{cumulative distribution function} factors as
        \[ F_{X_1, \dots, X_n}(x_1, \dots, x_n) = \prod_{i=1}^{n} F_{X_i}(x_i) \]
        for all $x_1, \dots, x_n \in \mathbb{R}$, where $F_{X_i}(x) = P(X_i \leq x)$ is the cumulative distribution function of $X_i$.

        \@{Events} $A_1, \dots, A_n$ are said to be \textbf{mutually independent} if
        \[ P\left( \bigcap_{i \in S} A_i \right) = \prod_{i \in S} P(A_i) \]
        for every $S \subseteq [n]$ with $|S| \geq 2$, where $[n] = \{1, \dots, n\}$ and $|S|$ denotes the number of elements of $S$.

        An infinite collection is said to be mutually independent if every finite subfamily is.
\end{definition}
...
\begin{theorem}[Law of Large Numbers]\label{law-of-large-numbers}
	Suppose $X_1, X_2, \dots$ are \@{independent and identically distributed} \@{random variables} with \@{finite} \@{mean} $\mu.$ Then for any $\epsilon > 0,$
...
If $X$ is a random variable, then its \textbf{moment generating function} is a real-valued function on the \@{reals} defined as
...
\begin{theorem}[Moment Generating Function of a Sum]\label{mgf-of-sum}
The \@{moment-generating-function} for the sum of two random variables is the product of the moment generating functions of the two random variables. More generally, if $X_1, X_2, \dots X_n$ are independent random variables, then
...
\subsubsection{Continuity Theorem For Moment Generating Functions}
...
\begin{theorem}[Continuity Theorem for Moment Generating Functions]\label{mgf-continuity-theorem}
Let $\{X_n\}$ be a \@{sequence} of random variables with moment generating functions defined by
...
\begin{theorem}[Central Limit Theorem]\label{central-limit-theorem}
If $c$ is a real constant and $X_1, X_2, \dots$ are independent random variables with mean $\mu$ and standard deviation $\sigma,$ then
