...
\title{Some Discrete Probability Distbutions}
...
The number $X$ of successes of a Hypergeometric experiment is called a \textbf{Hypergeometric random vairable,} and its probability distrbution is called the \textbf{Hypergeometric distribution.}
...
\begin{enumerate}
\item The probability that a single outcome will occurr within a small region is proportional to the size of the region and does not depend on the number of outcomes occurring outside the region, or the relative position of the region.
\end{enumerate}
...
\emph{Theorem:} Let $X$ be a binomial random variable with probability distribution $b(x, n, p).$ When $n \to \infty,$ $p \to 0,$ and $\lim_{n \to \infty}{np} = \mu$ remains constant,
...
\subsection{Negative Binomial Distirbution}

The \textbf{negative binomial random variable} generalizes the geometric random variable by giving the probability that it takes $n$ independent trials to get $r$ successes. Its pmf is
