...
\begin{definition}[Conditionally Independent]
	Let $A, B$ and $C$ be \@{events}. $A$ and $B$ are said to be \textbf{conditionally independent} given $C$ if and only if $P(C) > 0$ and $P(A | B, C) = P(A|C).$

	Equivalently, conditional independence may be stated as $P(A, B|C) = P(A|C)P(B|C),$ where $P(A, B|C)$ is the \@{joint probability} of $A$ and $B$ given $C.$
\end{definition}
