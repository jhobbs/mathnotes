...
A \textbf{stochastic matrix} (actually, a right stochastic matrix) is a \@{square-matrix} $A = (a_{ij}) \in \mathbb{R}^{n \times n}$ with nonnegative \@{entries} whose rows each sum to one:
...
A \textbf{left stochastic matrix} is a \@{square-matrix} $A = (a_{ij}) \in \mathbb{R}^{n \times n}$ with nonnegative \@{entries} whose columns each sum to one:
