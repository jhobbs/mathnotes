...
When the \dref{markov-chain} is \dref{ergodic}, the limit
...
A \textbf{stochastic matrix} (actually, a right stochastic matrix) is a \dref{square-matrix} $A = (a_{ij}) \in \mathbb{R}^{n \times n}$ with nonnegative \dref{entries} whose rows each sum to one:
...
A \textbf{left stochastic matrix} is a \dref{square-matrix} $A = (a_{ij}) \in \mathbb{R}^{n \times n}$ with nonnegative \dref{entries} whose columns each sum to one:
...
A \dref{stochastic-matrix} can be used to describe the transitions of a \dref{markov-chain}.

\begin{definition}[Doubly Stochastic Matrix]\synonyms{"bistochastic matrix"}
If a \dref{matrix} is both a \dref{row-stochastic-matrix} and a \dref{column-stochastic-matrix}, it is said to be a \textbf{doubly stochastic matrix.} That is, both its rows and its columns sum to 1.
\end{definition}

\begin{note}
A \dref{doubly-stochastic-matrix} is a \dref{convex-combination} of \dref{permutation-matrices}.
\end{note}

A \dref{stochastic-matrix} maps a probability vector to another probability vector — but only for the matching multiplication convention. A \dref{row-stochastic-matrix} preserves the sum acting on the right, $\vec{x}^\top A$; a \dref{column-stochastic-matrix} preserves it acting on the left, $A\vec{x}$; and a \dref{doubly-stochastic-matrix} preserves it either way. The demo below lets you apply each type to a probability vector, build doubly-stochastic matrices as convex combinations of permutation matrices, and iterate the map toward its stationary distribution.
