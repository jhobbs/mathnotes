...
\title{Some Continuous Probability Distributions}
...
\section{Some Continuous Probability Distributions}

\subsection{Uniform Distribution}

\begin{definition}[Uniform Distribution]
The \textbf{uniform distribution} has a "flat" density function and thus a uniform probability in a closed \@{interval} $[A, B]$.
...
The \textbf{normal distribution} or \textbf{Gaussian distribution} is the classic bell-shaped distribution.
...
The \textbf{standard normal distribution} is the distribution of a normal \@{random-variable} with mean $0$ and variance $1.$ We can transform a normal random variable $X$ into a standard normal random variable by
...
\begin{theorem}[Normal Approximation to the Binomial]\label{normal-approximation-binomial}
If $X$ is a \@[binomial]{binomial-distribution} random variable with mean $\mu = np$ and variance $\sigma^2 = npq,$ then
...
\begin{note}[Continuity Correction]\label{continuity-correction}
The $0.5$ is called a \textbf{continuity correction} and comes from the fact that
