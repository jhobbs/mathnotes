...
\begin{definition}[Null and Alternative Hypotheses]\label{null-alternative-hypothesis}
We could say that our \textbf{null hypothesis} is that the population mean is what we expect:

\[ H_0: \mu = \mu_0, \]

and that the \textbf{alternative hypothesis} is that it is different from what we expect:
...
\begin{definition}[p-value]\label{p-value}
The \textbf{p-value} is the probability of getting a test statistic ($z$) this extreme or more extreme if the actual mean is $\mu_0.$
\end{definition}
