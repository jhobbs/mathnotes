...
A \textbf{random variable} is a \dref{function} that associates a \dref{real} number with each \dref{element} in the \dref{sample-space}. For \dref{sample-space} $S$, we have $X: S \to \mathbb{R}$ and for $s \in S$, we have $x = X(s),$ where $x$ is a \dref{value} of the random variable.
:::

Random variables don't have to map to all reals; for example, \textbf{Bernoulli random variables} map to either 0 or 1.

Returning to sample spaces briefly, if a sample space contains a finite number of possibilities, or an unending sequence with as many elements as there are whole numbers (countably infinite), it is called a \textbf{discrete sample space}.
