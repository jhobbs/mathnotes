...
A \textbf{random variable} is a \@{function} that associates a \@{real} number with each \@{element} in the \@{sample-space}. For \@{sample-space} $S$, we have $X: S \to \mathbb{R}$ and for $s \in S$, we have $x = X(s),$ where $x$ is a \@{value} of the random variable.
:::
