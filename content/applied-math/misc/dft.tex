...
\section{Preliminary rambling notes}

\begin{enumerate}
\item Take a closed contour $C.$
\end{enumerate}

\begin{enumerate}
\item Take $N$ sample points along it, separated evenly by arc length.
\end{enumerate}

\begin{enumerate}
\item Calculate the Fourier coefficient $c_n$ at each of the $N$ points. These correspond to the frequencies $n = -[N/2], \dots, [N/2].$ We're basically measuring how much of each frequency the contour has at each of the $N$ different points.
\end{enumerate}
...
\begin{enumerate}
\item Only about half of these are unique because the frequency $N - m, m < N$ is the same as $-m$ (just like moving back 2 hours from 12 on a clock to 10 it the same as moving forward by 10 hours).
\end{enumerate}

\begin{enumerate}
\item Each $c_n$ represents a vector in the complex plane with:
\end{enumerate}
...
\begin{itemize}
\item phase angle = $\Arg(c_n)$
\end{itemize}

\begin{itemize}
\item rotation speed = $n$ 
\end{itemize}
...
\includedemo{dft-computation}

\subsection{Math on the Unit Circle}

The $n$th roots of \dref{unity}. There are $n$ $nth$ roots of 1. So, $z^{1} = 1$ has 1 solution, $1$ itself. $z^{2} = 1$ has 2 solutions, $1$ and $-1.$ Note that $-1$ is on the opposite side of the unit circle from $1.$ $z^3$ has 3 solutions: $1, e^{2 \pi / 3}$ and $e^{4 \pi / 3}.$  The general pattern is that the $n$th roots of unity are given by
...
Note that the resultant product is a root of unity as well, and in general the $nth$ roots of unit form a \dref{cyclic} \dref{abelian} \dref{group} under multiplication.
...
so $\phi$ is a \dref{group-homomorphism}.

Now,  $e_{\mu_n}$ is 1, that is, $e^{2 \pi i (0) / n},$ so $\ker{\phi} = {0}.$ But, $0$ is $e_{\mathbb{Z}_n},$ and so $\phi$ is \dref{injective}, and an \dref{isomorphism}, and $\mu_n$ and $\mathbb{Z}_n$ are isomorphic.
\end{proof}
...
\subsection{Sample Points as Vectors in $\mathbb{C}^n.$}

Let's say we have a contour on the complex plane, and we take $n$ sample points on it, in order, so $z_0, z_1, \dots, z_n.$ We can view these sample points (which form a \dref{sequence}) as forming a vector in $\mathbb{C}^n:$
...
It's easy to see that these basis vectors are \dref{orthogonal} (and therefore \dref{linearly-independent}); to see that they span $\mathbb{C}^n$, suppose $w = ( w_1, w_2, \dots, w_n. ).$ Then, $w_1 * e_1 + w_2 * e_2 + \cdots + w_n * e_n = w,$ that is, the components of $w$ (which are complex numbers) are exactly the scalar coefficients we need to multiply by our basis vectors to form $w,$ so this basis spans $\mathbb{C}^n.$

\subsubsection{$\mathbb{C}^n$ is an Inner Product Space}

Now, we can equip this vector space with an \dref{inner-product}

\begin{theorem}[$\mathbb{C}^N$ is an inner product space]\label{complex-inner-product-space}
The \dref{function}

\[ \langle z, w \rangle = \sum_{k=0}^{N-1} z_k \overline{w_k} \]

defined on $z, w \in \mathbb{C}^N$ is an \dref{inner-product}, and so $\mathbb{C}^N$ ($N$ a positive integer) forms an \dref{inner-product-space}.
\end{theorem}

\begin{proof}
Since $\mathbb{C}$ is a \dref{field}, $\mathbb{C}^N$ is obviously a \dref{vector-space}.

To see that this is indeed an inner product, we'll show the required properties hold.

For \dref{conjugate-symmetry}, let $z_k = a + bi$ and let $w_k = c + di.$ Then,
...
For \dref{linearity} in the first argument, let $w, v, z \in \mathbb{C}^n, a, b \in \mathbb{C}.$ Then
...
Now, to show \dref{positive-definiteness} let $z = a + bi.$ Then,
...
\begin{theorem}[Fourier Basis]\label{fourier-basis}
...
From \dref{finite-geometric-series}, we have that
...
Thus, $\langle \phi_m, \phi_n \rangle = 0,$ and so when $m \neq n,$ $\phi_m$ an $\phi_n$ are \dref{orthogonal} and therefore \dref{linearly-independent}.
...
\dref{Linearity} in the first argument of the \dref{inner-product} allows us to rewrite the right side to get
...
Is just the \dref{scalar-projection} of $\vec{u}$ onto $\phi_m.$ It is a complex number, and $|c_m|$ is proportional to the amplitude of the frequency $m$ present in $\vec{u},$ while $\Arg{c_m}$ gives the phase of frequency $m$ in $\vec{u}.$
