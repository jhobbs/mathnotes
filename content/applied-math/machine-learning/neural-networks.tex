...
\begin{itemize}
\item $W_{\ell} \in \mathbb{R}^{n_{\ell} \times n_{\ell - 1}}$ is the weight matrix (with the $i$th row representing the weights for the inputs to the $i$th \@{neuron} in the layer),
\end{itemize}

\begin{itemize}
\item $\vec{b}_{\ell} \in \mathbb{R}^{n_{\ell}}$ is the bias vector (with the $i$th entry representing the bias on the $i$th \@{neuron} in the layer,
\end{itemize}

\begin{itemize}
\item $\phi_{\ell} : \mathbb{R} \to \mathbb{R} $ is an activation function, applied \@{componentwise}, 
\end{itemize}
