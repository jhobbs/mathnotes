...
\textbf{Binary Classification} is the task of putting things into one of two categories (each called a \@{class}.)
\end{definition}

\begin{definition}[Target Class]
In \@{binary-classification}, the task is to determine whether or not an object belongs to a \@{class}, which we call the \textbf{target class}.
\end{definition}
...
Then, the job of machine learning in binary classification is to produce a good function $f.$ This is called \textbf{model training} or \textbf{learning.} More formally, if $f_\theta(x) : \reals^n \to {0,1}$ is a parameterized function, model training is the task of minimizing loss, that is, we want to find the set of parameters $\theta$ that minimizes
...
Applying a trained model $f_\theta$ to an unseen $\vec{x}$ to produce a predicted \@{label} is called \textbf{inference} or \textbf{prediction}.
\end{definition}
