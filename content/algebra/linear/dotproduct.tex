...
\[ \cos{\theta} =  \frac{\vec{u}}{\lVert\vec{u}\rVert} \cdot \frac{\vec{v}}{\lVert\vec{v}\rVert} = \frac{\vec{u} \cdot \vec{v}}{\lVert\vec{u}\rVert ~ \lVert\vec{v}\rVert} \] 

\[ \vec{u} \cdot \vec{v} = \lVert\vec{u}\rVert \lVert\vec{v}\rVert \cos{\theta} \]

\subsection{Properties of Dot Product}

For these properties, let $x, y, z \in \reals^n$ and $c \in \reals.$

\[ \vec{x} \cdot \vec{y} = \vec{y} \cdot \vec{x} ~ \forall x, y \in \reals^n \quad \text{(Commutative Property)} \]

\[ \vec{x} \cdot \vec{x} = {\lVert \vec{x} \rVert}^2 \geq 0, \quad \vec{x} \cdot \vec{x} = 0 ~ \text{iff} ~ \vec{x} = 0 \quad \text{(Property of Magnitude)} \] 
