...
\begin{enumerate}
\item $\langle R, + \rangle$ is an \dref{Abelian} \dref{group}.
\end{enumerate}
...
The integers, rationals, reals and complex numbers are all \dref{rings} with the usual addition and multiplication.
\end{example}
...
A ring doesn't have to have a \dref[multiplicative identity element]{unity}, but it can. If it has one, it's denoted $1$ and for all $a$ in $R$ satisfies $a1 = 1a = a$.
\end{remark}
...
The integers are not a \dref{field}, but the rationals, the reals, and the complex numbers are all fields.
\end{example}
