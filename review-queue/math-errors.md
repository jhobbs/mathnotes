...
- [x] probability-and-statistics/joint-probability.md:26 — multinomial "$P(X_1=n_1, \dots, X_n=n_r)$" — last variable should be $X_r$, not $X_n$. (fixed 2026-07-20)
- [x] probability-and-statistics/markov-chains.md:29 — "$\sum_{k=0}^{N} p_{jk} = 1$" sums $N+1$ terms but states are indexed $1,\dots,N$; sum should start at $k=1$. (fixed 2026-07-21)
...
- [x] probability-and-statistics/probability.md:33 — "the sum of the probabilities of all *events* in a sample space is 1" — events overlap ($2^{|S|}$ of them); it's the probabilities of all sample points/outcomes that sum to 1. (fixed 2026-07-20: both "all events" → "all sample points")
- [x] probability-and-statistics/limit-theorems.md:30 — "$E[X \mid X \ge t] > t$" should be $\ge t$ (equality possible); line 34 correctly uses $\ge$. (minor) (fixed 2026-07-21)
